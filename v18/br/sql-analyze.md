...
`ANALYZE` coleta estatísticas sobre os conteúdos das tabelas no banco de dados e armazena os resultados no catálogo do sistema [`pg_statistic`(catalog-pg-statistic.md "52.51. pg_statistic")]. Posteriormente, o planejador de consultas utiliza essas estatísticas para ajudar a determinar os planos de execução mais eficientes para as consultas.
...
Cada backend que executa `ANALYZE` informará seu progresso na visão `pg_stat_progress_analyze`. Consulte [Seção 27.4.1][(progress-reporting.md#ANALYZE-PROGRESS-REPORTING "27.4.1. ANALYZE Progress Reporting")] para obter detalhes.
