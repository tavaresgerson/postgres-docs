...
`REFRESH COLLATION VERSION`: Atualize a versão da collation do banco de dados. Consulte [Notas][(sql-altercollation.md#SQL-ALTERCOLLATION-NOTES "Notes")] para informações adicionais.
...
Também é possível vincular um padrão de sessão a um papel específico, em vez de a um banco de dados; veja [ALTER ROLE][(sql-alterrole.md "ALTER ROLE")]. As configurações específicas do papel substituem as específicas do banco de dados, se houver um conflito.
