...
Para os tipos de enumeração (descritos na Seção 8.7 [(datatype-enum.md "8.7. Enumerated Types")]), existem várias funções que permitem uma programação mais limpa, sem codificar valores específicos de um tipo de enumeração. Essas funções estão listadas na Tabela 9.35 [(functions-enum.md#FUNCTIONS-ENUM-TABLE "Table 9.35. Enum Support Functions")]. Os exemplos assumem um tipo de enumeração criado como:
