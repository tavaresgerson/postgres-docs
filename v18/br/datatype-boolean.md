...
O PostgreSQL fornece o tipo padrão SQL `boolean`; veja [Tabela 8.19][(datatype-boolean.md#DATATYPE-BOOLEAN-TABLE "Table 8.19. Boolean Data Type")]. O tipo `boolean` pode ter vários estados: “verdadeiro”, “falso” e um terceiro estado, “desconhecido”, que é representado pelo valor nulo do SQL.
...
<td>estado de verdadeiro ou falso</td>
</tr>
</tbody>
</table>
...
A função de saída de tipo de dados para o tipo `boolean` sempre emite `t` ou `f`, conforme mostrado no [Exemplo 8.2][(datatype-boolean.md#DATATYPE-BOOLEAN-EXAMPLE "Example 8.2. Using the boolean Type")].
...
As palavras-chave `TRUE` e `FALSE` são o método preferido (compatível com SQL) para escrever constantes booleanas em consultas SQL. Mas você também pode usar as representações de string seguindo a sintaxe de constante de literal de string genérica descrita em [Seção 4.1.2.7][(sql-syntax-lexical.md#SQL-SYNTAX-CONSTANTS-GENERIC "4.1.2.7. Constants of Other Types")], por exemplo `'yes'::boolean`.
