...
são usados para criar, descartar e transmitir alterações de um intervalo de replicação, respectivamente. Esses comandos só estão disponíveis através de uma conexão de replicação; eles não podem ser usados via SQL. Consulte [Seção 54.4](protocol-replication.md) para obter detalhes sobre esses comandos.
