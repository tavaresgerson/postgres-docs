...
As alterações podem ser consumidas utilizando o protocolo de replicação em streaming (consulte [Seção 54.4](protocol-replication.md) e [Seção 47.3](logicaldecoding-walsender.md)) ou chamando funções via SQL (consulte [Seção 47.4](logicaldecoding-sql.md)). Também é possível escrever métodos adicionais para consumir a saída de um slot de replicação sem modificar o código principal (consulte [Seção 47.7](logicaldecoding-writer.md)).