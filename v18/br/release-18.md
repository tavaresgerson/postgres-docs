...
* [pg_upgrade][(pgupgrade.md "pg_upgrade")] agora retém estatísticas do otimizador.
...
* A função [[`uuidv7()`][(functions-uuid.md#FUNC_UUID_GEN_TABLE "Table 9.45. UUID Generation Functions")]] para gerar UUIDs ordenados por timestamp.
...
* Suporte para [autenticação OAuth][(auth-oauth.md "20.15. OAuth Authorization/Authentication")].
* Suporte para `OLD` e `NEW` para cláusulas [[`RETURNING`][(dml-returning.md "6.4. Returning Data from Modified Rows")]] em comandos [INSERT][(sql-insert.md "INSERT")], [UPDATE][(sql-update.md "UPDATE")], [DELETE][(sql-delete.md "DELETE")] e [MERGE][(sql-merge.md "MERGE")].
...
* Altere [initdb][(app-initdb.md "initdb")] para habilitar verificações de checksum de dados (Greg Sabino Mullane) [§][(https://postgr.es/c/04bec894a04)]
...
[psql][(app-psql.md "psql")] ainda tratará `\.` como um marcador de fim de arquivo ao ler arquivos CSV a partir de `STDIN`. Clientes mais antigos do psql que se conectam a servidores PostgreSQL 18 podem experimentar problemas com `\copy`(app-psql.md#APP-PSQL-META-COMMANDS-COPY). Esta versão também exige que `\.` apareça sozinho em uma linha. * Não permita tabelas particionadas não registradas (Michael Paquier) [§][(https://postgr.es/c/e2bab2d79)]
...
Anteriormente, esses gatilhos eram executados com o papel ativo no momento da execução do gatilho (por exemplo, em [COMMIT][(sql-commit.md "COMMIT")]. Isso é significativo para casos em que o papel é alterado entre o tempo da fila e o compromisso da transação.
* Remova o suporte não funcional para privilégios de regra em [GRANT][(sql-grant.md "GRANT")]/[REVOKE][(sql-revoke.md "REVOKE")] (Fujii Masao) [§][(https://postgr.es/c/fefa76f70)]
...
* Altere [pesquisa de texto completo][(textsearch.md "Chapter 12. Full Text Search")] para usar o provedor de collation padrão do clúster para ler arquivos de configuração e dicionários, em vez de sempre usar libc (Peter Eisentraut) [§][(https://postgr.es/c/fb1a18810f0)]
...
* Converta alguns [`IN (VALUES ...)`][(functions-comparisons.md#FUNCTIONS-COMPARISONS-IN-SCALAR "9.25.1. IN")] em `x = ANY ...` para estatísticas de otimizador melhores (Alena Rybakina, Andrei Lepikhov) [§][(https://postgr.es/c/c0962a113)]
* Permita a transformação de [`OR`][(functions-logical.md "9.1. Logical Operators")] em matrizes para processamento de índice mais rápido (Alexander Korotkov, Andrey Lepikhov) [§][(https://postgr.es/c/ae4569161)]
* Acelere o processamento de [`INTERSECT`][(sql-select.md#SQL-INTERSECT "INTERSECT Clause")]], [`EXCEPT`][(sql-select.md#SQL-EXCEPT "EXCEPT Clause")]], agregados de janela][(tutorial-window.md "3.5. Window Functions")] e aliases de coluna de vista][(sql-createview.md "CREATE VIEW")] (Tom Lane, David Rowley) [§][(https://postgr.es/c/52c707483)] [§][(https://postgr.es/c/276279295)] [§][(https://postgr.es/c/8d96f57d5)] [§][(https://postgr.es/c/908a96861)]
* Permita que as chaves de [`SELECT DISTINCT`][(sql-select.md#SQL-DISTINCT "DISTINCT Clause")] sejam reordenadas internamente para evitar ordenação (Richard Guo) [§][(https://postgr.es/c/a8ccf4e93)]
...
* Ignore as colunas [`GROUP BY`][(sql-select.md#SQL-GROUPBY "GROUP BY Clause")] que são funcionalmente dependentes de outras colunas (Zhang Mingli, Jian He, David Rowley) [§][(https://postgr.es/c/bd10ec529)]
...
* Permita a omissão de varreduras de índices de funções de [btree][(xfunc-sql.md "36.5. Query Language (SQL)](Peter Geoghegan) [§][(https://postgr.es/c/92fe23d93)][(https://postgr.es/c/8a510275d)
...
Essa informação aparece no log do servidor, nas visualizações `pg_stat_progress_vacuum`(progress-reporting.md#VACUUM-PROGRESS-REPORTING "27.4.5. VACUUM Progress Reporting") e [`pg_stat_progress_analyze`(progress-reporting.md#PG-STAT-PROGRESS-ANALYZE-VIEW "Table 27.38. pg_stat_progress_analyze View") e a saída de [VACUUM](sql-vacuum.md "VACUUM") e [ANALYZE](sql-analyze.md "ANALYZE") quando no modo `VERBOSE`; o rastreamento deve ser habilitado com a variável do servidor [track_cost_delay_timing](runtime-config-statistics.md#GUC-TRACK-COST-DELAY-TIMING).
...
O mesclagem é usado por [pg_stat_statements][(pgstatstatements.md "F.32. pg_stat_statements — track statistics of SQL planning and execution")].
...
* Adicione suporte para o método de autenticação [OAuth][(auth-oauth.md "20.15. OAuth Authorization/Authentication")] (Jacob Champion, Daniel Gustafsson, Thomas Munro) [§][(https://postgr.es/c/b3f0be788)]
...
* Permitir a especificação do número fixo de tuplas mortas que acionará um [autovacuum][(routine-vacuuming.md#AUTOVACUUM "24.1.6. The Autovacuum Daemon")] (Nathan Bossart, Frédéric Yhuel) [§][(https://postgr.es/c/306dc520b)]
...
* Permita que [VACUUM][(sql-vacuum.md "VACUUM")] e [ANALYZE][(sql-analyze.md "ANALYZE")] processem tabelas particionadas sem processar seus filhos (Michael Harris) [§][(https://postgr.es/c/62ddf7ee9)]
...
O restabelecimento de um [pg_dump][(app-pgdump.md "pg_dump")], também utilizado por [pg_upgrade][(pgupgrade.md "pg_upgrade")], falhará se essas exigências não forem atendidas; mudanças de esquema devem ser feitas para que esses métodos de atualização tenham sucesso.
...
##### E.5.3.2.2. [COPIAR][(sql-copy.md "COPY")][[#][(#RELEASE-18-COPY)]
...
##### E.5.3.2.3. [EXPLICAR][(sql-explain.md "EXPLAIN")] [#][(#RELEASE-18-EXPLAIN)]
...
* Melhorar o mapeamento e a conversão de mapeamento de maiúsculas completas [Unicode][(collation.md#COLLATION-MANAGING-STANDARD "23.2.2.1. Standard Collations")] (Jeff Davis) [§][(https://postgr.es/c/4e7f62bc3)][§][(https://postgr.es/c/286a365b9)]
...
* Melhore os códigos de erro [`XML`(datatype-xml.md "8.13. XML Type") para se alinhar mais de perto ao padrão SQL (Tom Lane) [§](https://postgr.es/c/cd838e200)
...
* Adicione o parâmetro de conexão libpq [parâmetros][(libpq-connect.md#LIBPQ-CONNECT-SSL-MAX-PROTOCOL-VERSION)] e [variáveis de ambiente][(libpq-envars.md "32.15. Environment Variables")] para especificar a versão mínima e máxima aceitável do protocolo para conexões (Jelte Fennema-Nio) [§][(https://postgr.es/c/285613c60)] [§][(https://postgr.es/c/507034910)]
...
* Altere [initdb][(app-initdb.md "initdb")] para permitir o cálculo de verificações (Greg Sabino Mullane) [§][(https://postgr.es/c/983a588e0)] [§][(https://postgr.es/c/04bec894a)]
...
A opção `--no-sync` de initdb ainda está disponível para evitar a sincronização de quaisquer arquivos.
Adicione a opção (app-vacuumdb.md "vacuumdb") [`--missing-stats-only`] para calcular apenas as estatísticas do otimizador ausentes (Corey Huinker, Nathan Bossart) [§][(https://postgr.es/c/edba754f0)] [§][(https://postgr.es/c/987910502)]
...
* Permita que [pg_verifybackup][(app-pgverifybackup.md "pg_verifybackup")] verifique backups no formato tar (Amul Sul) [§][(https://postgr.es/c/8dfd31290)]
* Se o `--source-server` do [pg_rewind][(app-pgrewind.md "pg_rewind")] especificar um nome de banco de dados, use-o na saída do `--write-recovery-conf` (Masahiko Sawada) [§][(https://postgr.es/c/4ecdd4110)]
* Adicione a opção [pg_resetwal][(app-pgresetwal.md "pg_resetwal")] ao `--char-signedness` para alterar a assinatura padrão do `char` (Masahiko Sawada) [§][(https://postgr.es/c/30666d185)]
...
* Adicione a opção [pg_createsubscriber][(app-pgcreatesubscriber.md "pg_createsubscriber")] `--all` para criar réplicas lógicas para todos os bancos de dados (Shubham Khanna) [§][(https://postgr.es/c/fb2ea12f4)]
...
* Adicione a opção [pg_recvlogical][(app-pgrecvlogical.md "pg_recvlogical")] `--enable-failover` para especificar slots de falha (Hayato Kuroda) [§][(https://postgr.es/c/cf2655a90)]
...
* Separe a carga e a execução dos pontos de [injeção][(xfunc-c.md#XFUNC-ADDIN-INJECTION-POINTS "36.10.14. Injection Points")] (Michael Paquier, Heikki Linnakangas) [§][(https://postgr.es/c/4b211003e)] [§][(https://postgr.es/c/a0a5869a8)]
...
A função `pg_numa_available()`(functions-info.md#FUNCTIONS-INFO-SESSION-TABLE "Table 9.71. Session Information Functions") relata a consciência NUMA, e as visões do sistema `pg_shmem_allocations_numa`(view-pg-shmem-allocations-numa.md "53.28. pg_shmem_allocations_numa") e [`pg_buffercache_numa`(pgbuffercache.md#PGBUFFERCACHE-PG-BUFFERCACHE-NUMA "F.25.2. The pg_buffercache_numa View") que relatam a distribuição da memória compartilhada em nós NUMA.
* Adicione a tabela [TOAST](storage-toast.md "66.2. TOAST") ao [`pg_index`(catalog-pg-index.md "52.26. pg_index") para permitir índices de expressão muito grandes (Nathan Bossart) [§](https://postgr.es/c/b52c4fc3c)
* Remova a coluna [`pg_attribute`(catalog-pg-attribute.md "52.7. pg_attribute").`attcacheoff` (David Rowley) [§](https://postgr.es/c/02a8d0c45)
* Adicione a coluna [`pg_class`(catalog-pg-class.md "52.11. pg_class").`relallfrozen` (Melanie Plageman) [§](https://postgr.es/c/99f8f3fbb)
* Adicione [`amgettreeheight`(indexam.md "Chapter 63. Index Access Method Interface Definition"), `amconsistentequality`, e `amconsistentordering` ao método de acesso ao índice API (Mark Dilger) [§](https://postgr.es/c/56fead44d) [§](https://postgr.es/c/af4002b38)
...
* Adicione a extensão [pg_logicalinspect][(pglogicalinspect.md "F.28. pg_logicalinspect — logical decoding components inspection")] para inspecionar instantâneos lógicos (Bertrand Drouvot) [§][(https://postgr.es/c/7cdfeee32)]
* Adicione a extensão [pg_overexplain][(pgoverexplain.md "F.29. pg_overexplain — allow EXPLAIN to dump even more details")] que adiciona detalhes de depuração ao [`EXPLAIN`][(sql-explain.md "EXPLAIN")] de saída (Robert Haas) [§][(https://postgr.es/c/8d5ceb113)]
* Adicione colunas de saída ao [`postgres_fdw_get_connections()`][(postgres-fdw.md#POSTGRES-FDW-FUNCTIONS "F.38.2. Functions")] (Hayato Kuroda, Sagar Dilip Shedge) [§][(https://postgr.es/c/c297a47c5)] [§][(https://postgr.es/c/857df3cef)] [§][(https://postgr.es/c/4f08ab554)] [§][(https://postgr.es/c/fe186bda7)]
...
* Faça com que [pgbench][(pgbench.md "pgbench")] informe o número de transações que falharam, foram repetidas ou ignoradas em relatórios por script (Yugo Nagata) [§][(https://postgr.es/c/cae0f3c40)]
* Adicione a variável de servidor [isn][(isn.md "F.20. isn — data types for international standard numbers (ISBN, EAN, UPC, etc.)]] `weak` para controlar a aceitação de dígito de verificação inválido (Viktor Holmberg) [§][(https://postgr.es/c/448904423)]

Isso era anteriormente controlado apenas pela função `isn_weak()`.
* Permita que os valores sejam ordenados para acelerar a construção do índice de [btree_gist][(btree-gist.md "F.8. btree_gist — GiST operator classes with B-tree behavior")] (Bernd Helmle, Andrey Borodin) [§][(https://postgr.es/c/e4309f73f)]
* Adicione a função de verificação [amcheck][(amcheck.md "F.1. amcheck — tools to verify table and index consistency")] `gin_index_check()` (amcheck.md#AMCHECK-FUNCTIONS "F.1.1. Functions") para verificar os índices `GIN` (Grigory Kryachko, Heikki Linnakangas, Andrey Borodin) [§][(https://postgr.es/c/14ffaece0)]
...
A função existente `pg_buffercache_evict()` Function]") agora retorna o status de esvaziamento do buffer. * Permita que extensões instalem opções personalizadas de [EXPLAIN][(sql-explain.md "EXPLAIN")] (Robert Haas, Sami Imseih) [§][(https://postgr.es/c/c65bc2e1d)] [§][(https://postgr.es/c/4fd02bf7c)] [§][(https://postgr.es/c/50ba65e73)] * Permita que extensões usem a API de estatísticas acumuladas do servidor (Michael Paquier) [§][(https://postgr.es/c/7949d9594)] [§][(https://postgr.es/c/2eff9e678)]
