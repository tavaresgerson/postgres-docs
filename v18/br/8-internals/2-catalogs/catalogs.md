## Capítulo 52. Catálogos do sistema
...
Os catálogos do sistema são o local onde um sistema de gerenciamento de banco de dados relacional armazena metadados do esquema, como informações sobre tabelas e colunas, e informações de contabilidade interna. Os catálogos do PostgreSQL são tabelas regulares. Você pode excluir e recriar as tabelas, adicionar colunas, inserir e atualizar valores, e, dessa forma, causar sérios problemas no seu sistema. Normalmente, não se deve alterar os catálogos do sistema manualmente, pois normalmente existem comandos SQL para isso. (Por exemplo, `CREATE DATABASE` insere uma linha no catálogo `pg_database` — e, na verdade, cria o banco de dados no disco.) Existem algumas exceções para operações particularmente esotéricas, mas muitas dessas foram disponibilizadas como comandos SQL ao longo do tempo, e, portanto, a necessidade de manipulação direta dos catálogos do sistema está cada vez menor.