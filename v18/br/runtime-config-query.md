...
Consulte [Seção 5.12.5][(ddl-partitioning.md#DDL-PARTITIONING-CONSTRAINT-EXCLUSION "5.12.5. Partitioning and Constraint Exclusion")] para obter mais informações sobre o uso da exclusão de restrições para implementar a partição.
...
Definir esse valor como [geqo_threshold][(runtime-config-query.md#GUC-GEQO-THRESHOLD)] ou mais pode desencadear o uso do planejador GEQO, resultando em planos não ótimos. Veja [Seção 19.7.3][(runtime-config-query.md#RUNTIME-CONFIG-QUERY-GEQO "19.7.3. Genetic Query Optimizer")].

`jit` (`boolean`) [#](#GUC-JIT): Determina se a compilação JIT pode ser usada pelo PostgreSQL, se disponível (consulte [Capítulo 30][(jit.md "Chapter 30. Just-in-Time Compilation (JIT)]]). O padrão é `on`.
