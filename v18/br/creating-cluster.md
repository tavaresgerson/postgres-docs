...
Isso pode ser mais intuitivo se você estiver usando `pg_ctl` para iniciar e parar o servidor (consulte [Seção 18.3][(server-start.md "18.3. Starting the Database Server")]), de modo que `pg_ctl` seja o único comando que você use para gerenciar a instância do servidor de banco de dados.
