...
O OAuth 2.0 é uma estrutura padrão da indústria, definida em [RFC 6749][(https://datatracker.ietf.org/doc/html/rfc6749)], para permitir que aplicativos de terceiros obtenham acesso limitado a um recurso protegido. O suporte ao cliente OAuth deve ser habilitado quando o PostgreSQL é construído, consulte [Capítulo 17][(installation.md "Chapter 17. Installation from Source Code")] para mais informações.
...
`scope`: Uma lista separada por espaços dos escopos OAuth necessários para que o servidor autorize o cliente e autentique o usuário. Os valores apropriados são determinados pelo servidor de autorização e pelo módulo de validação OAuth utilizado (consulte o [Capítulo 50][(oauth-validators.md "Chapter 50. OAuth Validator Modules")] para obter mais informações sobre validadores). Este parâmetro é obrigatório.
...
`map`: Permite mapear entre os nomes de usuários do provedor de identidade OAuth e os nomes de usuários do banco de dados. Consulte [Seção 20.2][(auth-username-maps.md "20.2. User Name Maps")] para obter detalhes. Se um mapa não for especificado, o nome de usuário associado ao token (determinado pelo validador OAuth) deve corresponder exatamente ao nome do papel solicitado. Este parâmetro é opcional.
