...
Além disso, você pode forçar uma troca de segmento manualmente com `pg_switch_wal` se quiser garantir que uma transação recém-finalizada seja arquivada o mais rápido possível. Outras funções utilitárias relacionadas à gestão do WAL estão listadas em [Tabela 9.97][(functions-admin.md#FUNCTIONS-ADMIN-BACKUP-TABLE "Table 9.97. Backup Control Functions")].

Quando `wal_level` é `minimal`, alguns comandos SQL são otimizados para evitar o registro no WAL, conforme descrito em [Seção 14.4.7][(populate.md#POPULATE-PITR "14.4.7. Disable WAL Archival and Streaming Replication")]. Se a arquivamento ou replicação em fluxo estiverem ativadas durante a execução de uma dessas declarações, o WAL não conterá informações suficientes para a recuperação do arquivo de arquivação. (A recuperação em caso de falha não é afetada). Por essa razão, `wal_level` só pode ser alterado no início do servidor. No entanto, `archive_command` e `archive_library` podem ser alterados com uma recarga do arquivo de configuração. Se você está arquivando via shell e deseja interromper temporariamente o arquivamento, uma maneira de fazer isso é definir `archive_command` como a string vazia (`''`). Isso fará com que os arquivos do WAL se acumulem em `pg_wal/` até que um `archive_command` funcional seja reestabelecido.

### 25.3.2. Fazer um backup de base [#](#BACKUP-BASE-BACKUP)

A maneira mais fácil de realizar um backup básico é usar a ferramenta [pg_basebackup][(app-pgbasebackup.md "pg_basebackup")]. Ela pode criar um backup básico, seja em arquivos regulares ou em um arquivo tar. Se for necessário mais flexibilidade do que o [pg_basebackup][(app-pgbasebackup.md "pg_basebackup")] pode oferecer, você também pode fazer um backup básico usando a API de baixo nível (consulte [Seção 25.3.4][(continuous-archiving.md#BACKUP-LOWLEVEL-BASE-BACKUP "25.3.4. Making a Base Backup Using the Low Level API")]).
...
O arquivo de histórico de backup é apenas um pequeno arquivo de texto. Ele contém a string de rótulo que você deu para [pg_basebackup][(app-pgbasebackup.md "pg_basebackup")], além dos tempos de início e término e dos segmentos WAL do backup. Se você usou o rótulo para identificar o arquivo de dump associado, então o arquivo de histórico arquivado é suficiente para lhe dizer qual arquivo de dump deve ser restaurado.
...
Você pode usar [pg_basebackup][(app-pgbasebackup.md "pg_basebackup")] para fazer um backup incremental, especificando a opção `--incremental`. Você deve fornecer, como argumento para `--incremental`, o manifesto de backup de um backup anterior do mesmo servidor. No backup resultante, os arquivos não relacionados serão incluídos na íntegra, mas alguns arquivos relacionados podem ser substituídos por arquivos incrementais menores que contêm apenas os blocos que foram alterados desde o backup anterior e metadados suficientes para reconstruir a versão atual do arquivo.
...
Em vez de fazer um backup completo ou incremental usando [pg_basebackup][(app-pgbasebackup.md "pg_basebackup")], você pode fazer um backup de base usando a API de nível baixo. Esse procedimento contém alguns passos adicionais em relação ao método pg_basebackup, mas é relativamente simples. É muito importante que esses passos sejam executados em sequência e que o sucesso de um passo seja verificado antes de prosseguir para o próximo passo.

É possível executar múltiplos backups simultaneamente (tanto os iniciados usando essa API de backup quanto os iniciados usando [pg_basebackup][(app-pgbasebackup.md "pg_basebackup")]).
...
Assim como os backups básicos, a maneira mais fácil de produzir um backup quente independente é usar a ferramenta [pg_basebackup][(app-pgbasebackup.md "pg_basebackup")]. Se você incluir o parâmetro `-X` ao chamá-la, todos os registros de pré-escrita necessários para usar o backup serão incluídos automaticamente no backup, e não é necessária nenhuma ação especial para restaurar o backup.
