...
Um índice é, efetivamente, uma mapeo de alguns valores de chave de dados para *identificador de tupla*, ou TIDs, das versões de linha (tuplas) na tabela pai do índice. Um TID consiste em um número de bloco e um número de item dentro desse bloco (ver [Seção 66.6][(storage-page-layout.md "66.6. Database Page Layout")]). Esta é informação suficiente para obter uma versão específica de linha da tabela. Os índices não são diretamente conscientes de que, sob MVCC, pode haver múltiplas versões existentes da mesma linha lógica; para um índice, cada tupla é um objeto independente que precisa de sua própria entrada de índice. Assim, uma atualização de uma linha sempre cria novas entradas de índice para a linha, mesmo que os valores das chaves não tenham mudado. ([Tuples HOT][(storage-hot.md "66.7. Heap-Only Tuples (HOT)]) são uma exceção a esta declaração; mas os índices também não lidam com esses.) As entradas de índice para tuplas mortas são recuperadas (por vácuo) quando as próprias tuplas mortas são recuperadas.