...
onde a auto-referência recursiva deve aparecer no lado direito do `UNION`. Apenas uma auto-referência recursiva é permitida por consulta. As declarações de modificação de dados recursivas não são suportadas, mas você pode usar os resultados de uma consulta recursiva `SELECT` em uma declaração de modificação de dados. Veja [Seção 7.8][(queries-with.md "7.8. WITH Queries (Common Table Expressions)] para um exemplo.
...
Consulte [Seção 7.8][(queries-with.md "7.8. WITH Queries (Common Table Expressions)] para obter informações adicionais.
...
As funções de janela são descritas em detalhes em [Seção 3.5][(tutorial-window.md "3.5. Window Functions")], [Seção 4.2.8][(sql-expressions.md#SYNTAX-WINDOW-FUNCTIONS "4.2.8. Window Function Calls")] e [Seção 7.2.5][(queries-table-expressions.md#QUERIES-WINDOW "7.2.5. Window Function Processing")].
...
Observe a forma típica de consultas recursivas: uma condição inicial, seguida de `UNION`, seguida pela parte recursiva da consulta. Certifique-se de que a parte recursiva da consulta eventualmente retorne nenhum tuplo, caso contrário, a consulta entrará em um loop indefinidamente. (Veja [Seção 7.8][(queries-with.md "7.8. WITH Queries (Common Table Expressions)] para mais exemplos.)
