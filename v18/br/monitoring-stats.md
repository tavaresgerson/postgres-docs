...
Vários pontos de vista predefinidos, listados em [Tabela 27.1][(monitoring-stats.md#MONITORING-STATS-DYNAMIC-VIEWS-TABLE "Table 27.1. Dynamic Statistics Views")], estão disponíveis para mostrar o estado atual do sistema. Há também vários outros pontos de vista, listados em [Tabela 27.2][(monitoring-stats.md#MONITORING-STATS-VIEWS-TABLE "Table 27.2. Collected Statistics Views")], disponíveis para mostrar as estatísticas acumuladas. Alternativamente, é possível criar pontos de vista personalizados usando as funções estatísticas cumulativas subjacentes, conforme discutido em [Seção 27.2.26][(monitoring-stats.md#MONITORING-STATS-FUNCTIONS "27.2.26. Statistics Functions")].
...
<td>Uma linha por processo do servidor, mostrando informações relacionadas à atividade atual desse processo, como estado e consulta atual. Veja<a class="link" href="monitoring-stats.md#MONITORING-PG-STAT-ACTIVITY-VIEW" title="27.2.3. pg_stat_activity">
<code class="structname">
      pg_stat_activity
     </code>
</a>para obter mais informações.</td>
</tr>
<tr>
<td>
<code class="structname">
...
<td>Uma linha por processo de emissor WAL, mostrando estatísticas sobre a replicação para o servidor de standby conectado a esse emissor. Veja<a class="link" href="monitoring-stats.md#MONITORING-PG-STAT-REPLICATION-VIEW" title="27.2.4. pg_stat_replication">
<code class="structname">
      pg_stat_replication
     </code>
</a>para obter mais informações.</td>
...
<td>Apenas uma linha, mostrando estatísticas sobre o receptor WAL do servidor conectado desse receptor. Veja<a class="link" href="monitoring-stats.md#MONITORING-PG-STAT-WAL-RECEIVER-VIEW" title="27.2.6. pg_stat_wal_receiver">
<code class="structname">
      pg_stat_wal_receiver
     </code>
</a>para obter mais informações.</td>
...
<td>Apenas uma linha, mostrando estatísticas sobre os blocos pré-carregados durante a recuperação.<a class="link" href="monitoring-stats.md#MONITORING-PG-STAT-RECOVERY-PREFETCH" title="27.2.7. pg_stat_recovery_prefetch">
<code class="structname">
      pg_stat_recovery_prefetch
     </code>
</a>para obter mais informações.</td>
...
<td>Pelo menos uma linha por assinatura, mostrando informações sobre os trabalhadores da assinatura. Veja<a class="link" href="monitoring-stats.md#MONITORING-PG-STAT-SUBSCRIPTION" title="27.2.8. pg_stat_subscription">
<code class="structname">
      pg_stat_subscription
     </code>
</a>para obter mais informações.</td>
...
<td>Uma linha por conexão (regular e de replicação), mostrando informações sobre SSL usado nesta conexão. Veja<a class="link" href="monitoring-stats.md#MONITORING-PG-STAT-SSL-VIEW" title="27.2.10. pg_stat_ssl">
<code class="structname">
      pg_stat_ssl
     </code>
</a>para obter mais informações.</td>
...
<td>Uma linha por conexão (regular e de replicação), mostrando informações sobre a autenticação e criptografia GSSAPI usadas nesta conexão. Veja<a class="link" href="monitoring-stats.md#MONITORING-PG-STAT-GSSAPI-VIEW" title="27.2.11. pg_stat_gssapi">
<code class="structname">
      pg_stat_gssapi
     </code>
</a>para obter mais informações.</td>
...
<td>Uma linha para cada backend (incluindo processos de trabalho do autovacuum) em execução<code class="command">
     ANALYZE
    </code>, mostrando o progresso atual.<a class="xref" href="progress-reporting.md#ANALYZE-PROGRESS-REPORTING" title="27.4.1. ANALYZE Progress Reporting">Seção 27.4.1</a>
    .
   </td>
</tr>
<tr>
...
<td>Uma linha para cada backend em execução<code class="command">
     CREATE INDEX
    </code>ou<code class="command">
     REINDEX
    </code>, mostrando o progresso atual. Veja<a class="xref" href="progress-reporting.md#CREATE-INDEX-PROGRESS-REPORTING" title="27.4.4. CREATE INDEX Progress Reporting">Seção 27.4.4</a>
    .
   </td>
</tr>
<tr>
...
<td>Uma linha para cada backend (incluindo processos de trabalho do autovacuum) em execução<code class="command">
     VACUUM
    </code>, mostrando o progresso atual.<a class="xref" href="progress-reporting.md#VACUUM-PROGRESS-REPORTING" title="27.4.5. VACUUM Progress Reporting">Seção 27.4.5</a>
    .
   </td>
</tr>
<tr>
...
<td>Uma linha para cada backend em execução<code class="command">
     ...
     VACUUM FULL
    </code>, mostrando o progresso atual.<a class="xref" href="progress-reporting.md#CLUSTER-PROGRESS-REPORTING" title="27.4.2. CLUSTER Progress Reporting">Seção 27.4.2</a>
    .
   </td>
</tr>
<tr>
...
<td>Uma linha para cada processo de emissor WAL que está fazendo streaming de um backup de base,
       mostrando o progresso atual. Veja<a class="xref" href="progress-reporting.md#BASEBACKUP-PROGRESS-REPORTING" title="27.4.6. Base Backup Progress Reporting">Seção 27.4.6</a>
    .
   </td>
</tr>
<tr>
...
<td>Uma linha para cada backend em execução<code class="command">
     COPY
    </code>, mostrando o progresso atual.<a class="xref" href="progress-reporting.md#COPY-PROGRESS-REPORTING" title="27.4.3. COPY Progress Reporting">Seção 27.4.3</a>
    .
   </td>
</tr>
</tbody>
...
<td>
    ...
    , but counts only
      calls during the current transaction (which are
    <span class="emphasis">
<em>
      not
     </em>
...
<td>
    ...
    , except that only
      system sequences are shown.  (Presently, no system sequences are defined, so this view is always empty.)
   </td>
</tr>
<tr>
<td>
...
<code class="literal">
        ...
        : The backend is in initial startup. Client
...
<p>
     ...
     this
      field shows the identifier of the currently executing query. In all other states, it shows the identifier of last query that was executed.  Query identifiers are not computed by default so this field will be null unless
...
<p>
     ...
     this field shows the
       currently executing query. In all other states, it shows the last query that was executed. By default the query text is truncated at 1024 bytes; this value can be changed via the parameter
...
<td>
    The server process is waiting for exclusive access to
       a data buffer.  Buffer pin waits can be protracted if another process holds an open cursor that last read data from the buffer in question. See
...
<td>
    The server process is waiting for activity on a socket
       connected to a user application.  Thus, the server expects something to happen that is independent of its internal processes.
    <code class="literal">
     wait_event
    </code>
    will identify the specific wait point;
...
<td>
    The server process is waiting for a heavyweight lock.
       Heavyweight locks, also known as lock manager locks or simply locks, primarily protect SQL-visible objects such as tables.  However, they are also used to ensure mutual exclusion for certain internal operations such as relation extension.
    <code class="literal">
     wait_event
    </code>
...
<td>
    ...
    will contain a name identifying the purpose
       of the lightweight lock.  (Some locks have specific names; others are part of a group of locks each with a similar purpose.) See
...
<td>Esperando no loop principal do processo do arqiver.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no loop principal do processo de inicializador de autovacuum.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no processo de escritor de fundo, hibernando.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no loop principal do processo de escritor de fundo.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no loop principal do processo de checkpointer.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o processo de verificação seja concluído.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no loop principal do processo do trabalhador de E/S.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Aguardar no loop principal do processo de replicação lógica.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no loop principal do processo de inicializador de replicação lógica.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no loop principal do processo de aplicação paralela de replicação lógica.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no loop principal do processo de inicialização para o WAL chegar, durante a recuperação de streaming.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no loop principal do trabalhador de sincronização de slot.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o trabalhador de sincronização de intervalo seja desligado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no loop principal do processo syslogger.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no loop principal do processo do receptor WAL.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no loop principal do processo de emissor WAL.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no sumidor WAL para mais WAL serem gerados.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no loop principal do processo de escritor WAL.</td>
</tr>
</tbody>
</table>
...
<td>Esperando adquirir um pino exclusivo em um buffer.</td>
</tr>
</tbody>
</table>
...
<td>Esperando ler dados do cliente.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para escrever dados para o cliente.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando ler dados do cliente enquanto estabelece uma sessão GSSAPI.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no receptor WAL para estabelecer conexão com o servidor remoto.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando no receptor WAL para receber dados do servidor remoto.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando SSL enquanto tenta conexão.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o WAL seja recebido e descartado pelo standby físico.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o WAL seja descartado no processo de emissor de WAL.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperar por qualquer atividade ao processar respostas do receptor WAL no processo de emissor WAL.</td>
</tr>
</tbody>
</table>
...
<td>Esperando em uma extensão.</td>
</tr>
</tbody>
</table>
...
<td>Esperando que outro processo complete a IO.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando a execução de IO via io_uring.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando a submissão de IO via io_uring.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o backup de base leia de um arquivo.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que os dados escritos por um backup de base atinjam armazenamento durável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o backup de base escreva em um arquivo.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma leitura de um arquivo armazenado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um arquivo armazenado seja truncado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma escrita em um arquivo armazenado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma leitura do<code class="filename">
     pg_control
    </code>
    file.
   </td>
...
<td>Esperando pelo<code class="filename">
     pg_control
    </code>arquivo para alcançar armazenamento durável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma atualização para o<code class="filename">
     pg_control
    </code>arquivo para alcançar armazenamento durável.</td>
</tr>
<tr>
...
<td>Esperando uma escrita para o<code class="filename">
     pg_control
    </code>
    file.
   </td>
...
<td>Esperando por uma atualização para atualizar<code class="filename">
     pg_control
    </code>
    file.
   </td>
...
<td>Esperando por uma operação de cópia de arquivo.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma leitura durante uma operação de cópia de arquivo.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma escrita durante uma operação de cópia de arquivo.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o arquivo de dados de relacionamento seja estendido.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um arquivo de dados de relacionamento atinja armazenamento durável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma sincronização imediata de um arquivo de dados de relação para armazenamento durável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por um pré-pré-visualização assíncrona de um arquivo de dados de relação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma leitura de um arquivo de dados de relação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que os dados de uma relação atinjam o armazenamento permanente.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um arquivo de dados de relação seja truncado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma escrita em um arquivo de dados de relacionamento.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um segmento de memória compartilhada dinâmica seja alocado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando preencher um arquivo de suporte de memória compartilhada dinâmica com zeros.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma leitura enquanto adiciona uma linha ao arquivo de bloqueio do diretório de dados.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que os dados atinjam o armazenamento permanente enquanto adiciona uma linha ao arquivo de bloqueio do diretório de dados.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma escrita enquanto adiciona uma linha ao arquivo de bloqueio do diretório de dados.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler enquanto cria o arquivo de bloqueio do diretório de dados.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que os dados atinjam o armazenamento permanente enquanto cria o arquivo de bloqueio do diretório de dados.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma escrita enquanto cria o arquivo de bloqueio do diretório de dados.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma leitura durante a revalidação do arquivo de bloqueio do diretório de dados.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que as correspondências de reescrita lógica atinjam o armazenamento durável durante um ponto de verificação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que os dados de mapeamento atinjam armazenamento durável durante uma reescrita lógica.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma gravação de dados de mapeamento durante uma reescrita lógica.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que as mapearizações de reescrita lógica alcancem armazenamento durável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por o mapeamento de dados ser truncado durante uma reescrita lógica.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma descrição de mapeamentos de reescrita lógica.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma leitura do arquivo de mapa de relações.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando substituição durável de um arquivo de mapa de relação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma escrita no arquivo do mapa de relações.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma leitura durante a gestão do buffer de reordenação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma escrita durante a gestão do buffer de reordenação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma leitura de mapeamento lógico durante a gestão do buffer de reordenação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma leitura de um arquivo de controle de slot de replicação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o arquivo de controle de slot de replicação atinja o armazenamento durável enquanto o restaura à memória.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o arquivo de controle de slot de replicação atinja o armazenamento durável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma escrita em um arquivo de controle de slot de replicação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que os dados do SLRU atinjam armazenamento durável durante um ponto de verificação ou desligamento do banco de dados.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma leitura de uma página do SLRU.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que os dados do SLRU atinjam armazenamento durável após a escrita de uma página.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma escrita de uma página SLRU.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma leitura de um instantâneo de catálogo histórico serializado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um instantâneo serializado do catálogo histórico atinja o armazenamento durável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma descrição de um instantâneo de catálogo histórico serializado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um arquivo de histórico de cronologia recebido por replicação em streaming atinja armazenamento durável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma gravação de um arquivo de histórico de cronologia recebida por replicação em streaming.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma leitura de um arquivo de histórico de linha do tempo.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o arquivo de histórico de linha do tempo recém-criado atinja o armazenamento durável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma gravação de um arquivo de histórico de linha do tempo recém-criado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma leitura de um arquivo de estado de duas fases.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um arquivo de estado de duas fases atinja armazenamento durável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma escrita de um arquivo de estado de duas fases.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o arquivo de versão atinja o armazenamento durável enquanto cria um banco de dados.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o arquivo de versão seja escrito durante a criação de um banco de dados.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma leitura de um arquivo de histórico de linha de tempo durante um comando de linha de tempo walsender.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o WAL atinja armazenamento durável durante o bootstrapping.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma escrita de uma página WAL durante o arranque.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma leitura ao criar um novo segmento WAL ao copiar um existente.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por um novo segmento WAL criado copiando um segmento existente para alcançar armazenamento durável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperar por uma escrita ao criar um novo segmento WAL ao copiar um existente.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o arquivo WAL recém-inicializado atinja o armazenamento durável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma escrita enquanto inicializa um novo arquivo WAL.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma leitura de um arquivo WAL.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma leitura de um arquivo de resumo WAL.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma escrita em um arquivo de resumo WAL.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o arquivo WAL atinja o armazenamento durável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que os dados atinjam o armazenamento permanente enquanto se atribui um novo método de sincronização WAL.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando uma escrita em um arquivo WAL.</td>
</tr>
</tbody>
</table>
...
<td>Esperando por nós de subplano de um<code class="literal">
     Append
    </code>O nó do plano estará pronto.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por<a class="xref" href="runtime-config-wal.md#GUC-ARCHIVE-CLEANUP-COMMAND">
...
<td>Esperando por<a class="xref" href="runtime-config-wal.md#GUC-ARCHIVE-COMMAND">
     archive_command
    </a>para completar.</td>
</tr>
<tr>
...
<td>Esperando pelo término de outro backend.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por arquivos WAL necessários para que o backup seja arquivado com sucesso.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o trabalhador de fundo seja desligado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o trabalhador de fundo comece.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o número da página necessário para continuar um varredura em árvore B paralela esteja disponível.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o I/O de buffer seja concluído.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por um backend que bloqueie a conclusão de um ponto de verificação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por um backend que bloqueie o início de um ponto de verificação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um ponto de controle seja concluído.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o posto de controle comece.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por atividade de um processo de criança enquanto executa um<code class="literal">
     Gather
    </code>nó do plano.</td>
</tr>
<tr>
...
<td>Esperando que um participante Paralelo Hash eleito aloje uma tabela de hash.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando eleger um participante do Hash Paralelo para alocar uma tabela de hash.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que outros participantes do Hash Paralelo terminem de carregar uma tabela de hash.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um participante Paralelo Hash eleito aloje a tabela de hash inicial.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando eleger um participante do Hash Paralelo para alocar a tabela de hash inicial.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que outros participantes do Hash Paralelo terminem de hashar a relação interna.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que outros participantes do Hash Paralelo terminem a partição da relação externa.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando eleger um participante do Hash Paralelo para decidir sobre o crescimento do próximo lote.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando eleger um participante do Hash Paralelo para alocar mais lotes.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um participante do Hash Paralelo eleito decida sobre o crescimento do próximo lote.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um participante do Hash Paralelo eleito aloje mais lotes.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que outros participantes do Hash Paralelo terminem a redistribuição.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando eleger um participante do Hash Paralelo para alocar mais buckets.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um participante do Hash Paralelo eleito termine a alocação de mais buckets.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que outros participantes do Hash Paralelo terminem de inserir tuplas em novos buckets.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperar que um líder de replicação lógica aplique o processo para enviar dados para um processo de aplicação paralela.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por um processo de aplicação paralela de replicação lógica para alterar o estado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por um servidor remoto de replicação lógica para enviar dados para a sincronização inicial da tabela.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por um servidor remoto de replicação lógica para mudar de estado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que outro processo seja anexado a uma fila de mensagens compartilhada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para escrever uma mensagem de protocolo em uma fila de mensagens compartilhada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando receber bytes de uma fila de mensagens compartilhada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando enviar bytes para uma fila de mensagens compartilhada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por uma criação multixact para ser concluída.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que a varredura de bitmap paralela seja inicializada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando em paralelo<code class="command">
     CREATE INDEX
    </code>trabalhadores para finalizar a varredura do monte.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que os trabalhadores paralelos terminem de calcular.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o líder do grupo confirme o ID da transação no final da transação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um evento de barreira seja processado por todos os backends.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando promoção de standby.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando pela resolução do conflito de recuperação para uma limpeza a vácuo.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando pela resolução de conflitos de recuperação para a eliminação de um tablespace.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por<a class="xref" href="runtime-config-wal.md#GUC-RECOVERY-END-COMMAND">
...
<td>Esperando que a recuperação seja retomada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que a origem de replicação se torne inativa para que possa ser descartada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um slot de replicação se torne inativo para que possa ser descartado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por<a class="xref" href="runtime-config-wal.md#GUC-RESTORE-COMMAND">
     restore_command
    </a>para completar.</td>
</tr>
<tr>
...
<td>Esperando obter um instantâneo válido para um<code class="literal">
...
<td>Esperando confirmação de um servidor remoto durante a replicação sincronizada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o receptor WAL saia.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando pelo processo de inicialização para enviar dados iniciais para replicação em streaming.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que um novo resumo do WAL seja gerado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que o líder do grupo atualize o status da transação no final da transação.</td>
</tr>
</tbody>
</table>
...
<td>Esperando adquirir um bloqueio de usuário consultor.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando adquirir um bloqueio em uma transação remota que está sendo aplicada por um assinante de replicação lógica.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando estender uma relação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando atualizar<code class="structname">
     pg_database
    </code>
    .
    <code class="structfield">
     datfrozenxid
    </code>e<code class="structname">
     pg_database
    </code>
    .
    <code class="structfield">
...
<td>Esperando adquirir um bloqueio em um objeto de banco de dados não relacionado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando adquirir um bloqueio em uma página de uma relação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando adquirir um elo em uma relação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando adquirir um bloqueio de inserção especulativa.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que a transação termine.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando adquirir um bloqueio em uma tupla.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando adquirir um bloqueio de usuário.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando adquirir um bloqueio de ID de transação virtual; veja<a class="xref" href="transaction-id.md" title="67.1. Transactions and Identifiers">Seção 67.1</a>
    .
   </td>
</tr>
</tbody>
...
<td>Esperando gerenciar a alocação de espaço de uma extensão na memória compartilhada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que outro processo complete a IO via io_uring.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar a fila de submissão de trabalhadores AIO.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando atualizar o<code class="filename">
...
<td>Esperando para ler ou atualizar o estado atual dos trabalhadores do autovacuum.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para garantir que uma tabela selecionada para autovacuum ainda precise ser vacumada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar o estado do trabalhador de fundo.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar informações relacionadas ao vácuo para um índice de árvore B.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar uma página de dados na memória.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando associar um bloco de dados a um buffer no pool de buffers.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para gerenciar solicitações fsync.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar o último valor definido para um timestamp de comprovação de transação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por I/O em um buffer de marcação de tempo SLRU.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar o cache de marcação de tempo SLRU.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar o<code class="filename">
     pg_control
    </code>arquivo ou crie um novo arquivo WAL.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar o registro de memória compartilhada dinâmica.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar o alocador de memória compartilhada dinâmica do registro de memória compartilhada dinâmica.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar a tabela hash compartilhada do registro de memória compartilhada dinâmica.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar informações de alocação dinâmica de memória compartilhada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar informações relacionadas aos pontos de injeção.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar as informações de bloqueio de caminho rápido de um processo.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar informações sobre<span class="quote">“<span class="quote">pesado</span>”</span>
    locks.
   </td>
</tr>
<tr>
...
<td>Esperando acessar o alocador de memória compartilhada dinâmica do lançador de replicação lógica.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar a tabela de hash compartilhada do lançador de replicação lógica.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar o estado dos trabalhadores de replicação lógica.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar o estado multixact compartilhado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por I/O em um buffer SLRU de membro multixact.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar o cache multixact do membro SLRU.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por I/O em um buffer SLRU de múltiplos acasos.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar o cache SLRU de multixact offset.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou truncar informações multixact.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por I/O em um<code class="command">
     NOTIFY
    </code>mensagem SLRU buffer.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar<code class="command">
     NOTIFY
    </code>
    messages.
   </td>
...
<td>Esperando para atualizar o limite<code class="command">
     NOTIFY
    </code>armazenamento de mensagens.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar o<code class="command">
     NOTIFY
    </code>mensagem SLRU cache.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando alocar um novo OID.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando escolher o próximo subplano durante a execução do plano de Append paralelo.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando sincronizar os trabalhadores durante a execução do plano de varredura em árvore paralela.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando sincronizar os trabalhadores durante a execução do plano de junção hash paralela.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por alocação dinâmica de memória compartilhada de consulta paralela.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por alocação dinâmica de memória compartilhada de vácuo paralela.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar as informações de uma consulta paralela sobre tipos compostos.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar as informações de uma consulta paralela sobre modificadores de tipo que identificam tipos de registro anônimos.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar a lista de bloqueios preditivos mantidos pela transação serializável atual durante uma consulta paralela.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acesso aos dados de estatísticas de memória compartilhada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando pelo acesso do alocador de memória compartilhada dinâmica de estatísticas.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por estatísticas de acesso à tabela de hash de memória compartilhada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar as informações de bloqueio de predicado usadas por transações serializáveis.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar as estruturas de dados compartilhadas por processo (tipicamente, para obter um instantâneo ou relatar o ID de transação de uma sessão).</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar um<code class="filename">
     pg_filenode.map
    </code>arquivo (usado para rastrear as atribuições do filenode de certos catálogos do sistema).</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar um<code class="filename">
     pg_internal.init
    </code>arquivo de inicialização do cache de relação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Aguardar para criar, descartar ou usar uma origem de replicação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar o progresso de uma origem de replicação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando alocar ou liberar um slot de replicação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar o estado do slot de replicação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por I/O em um slot de replicação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por I/O em um buffer SLRU de conflito de transação serializável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar compartilhado<code class="filename">
     pg_serial
    </code>
    state.
   </td>
...
<td>Esperando acessar a lista de transações serializáveis concluídas.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar a lista de bloqueios preditivos mantidos por transações serializáveis.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar informações sobre transações serializáveis.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar o cache SLRU de conflitos de transação serializável.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar um bitmap compartilhado TID durante uma varredura paralela de índice de bitmap.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar um banco de tuplas compartilhado durante uma consulta paralela.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando encontrar ou alocar espaço na memória compartilhada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando recuperar mensagens da fila de invalidação do catálogo compartilhado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando adicionar uma mensagem à fila de invalidação do catálogo compartilhado.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por I/O em um buffer SLRU de subtransação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar o cache de subtransação SLRU.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar informações sobre o estado da replicação sincronizada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para selecionar a localização inicial de um varredura de tabela sincronizada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando criar ou descartar um espaço de tabela.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar o estado das transações preparadas.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar informações de eventos de espera personalizados.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando substituir uma página nos buffers do WAL.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando inserir dados WAL em um buffer de memória.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando para ler ou atualizar o estado de resumo do WAL.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando que os buffers de WAL sejam escritos no disco.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando atualizar os limites de identificação de transações e consumo multixact.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando por I/O em um buffer de status de transação SLRU.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando acessar o cache de status de transação SLRU.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando executar<code class="function">
     pg_xact_status
    </code>ou atualize o ID de transação mais antigo disponível.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando alocar um novo ID de transação.</td>
</tr>
</tbody>
</table>
...
<td>Esperar durante o backup de base quando a atividade é controlada.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando entre os registros enquanto realiza um ponto de verificação.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando devido a uma chamada<code class="function">
     pg_sleep
    </code>ou uma função de irmão.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando aplicar o WAL durante a recuperação devido a um atraso de configuração.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperar durante a recuperação quando os dados do WAL não estão disponíveis em nenhuma fonte (<code class="filename">
     pg_wal
    </code>, arquivar ou transmitir).</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando enquanto envia solicitações de sincronização para o checkpointer, porque a fila de solicitações está cheia.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperar enquanto adquire um spinlock confiante.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando em um ponto de atraso em vácuo baseado em custo.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando adquirir um bloqueio exclusivo para truncar quaisquer páginas vazias no final de uma tabela varrida.</td>
</tr>
<tr>
<td>
<code class="literal">
...
<td>Esperando após um erro do resumidor WAL.</td>
</tr>
</tbody>
</table>
...
<code class="literal">
        ...
        : This WAL sender's connected standby is
...
<code class="literal">
        ...
        : This WAL sender is streaming changes
...
<p>
     ...
     incurred while committing if this
...
<code class="literal">
        ...
        : This standby server is now asynchronous,
          but can potentially become synchronous if one of current synchronous ones fails.
       </p>
</li>
<li class="listitem">
<p>
...
<code class="literal">
        ...
        : This standby server is considered as a candidate
          for quorum standbys.
       </p>
</li>
</ul>
...
<p>
     ...
     . Streaming only
        works with top-level transactions (subtransactions can't be streamed independently), so the counter is not incremented for subtransactions.
    </p>
</td>
</tr>
<tr>
...
<p>Tipo do processo de trabalho da assinatura. Os tipos possíveis são<code class="literal">
      apply
...
<p>ID de processo da subscrição do processo do trabalhador</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>ID de processo do líder aplique ao trabalhador se este processo for um trabalhador de aplicação em paralelo; NULL se este processo for um trabalhador de aplicação em líder ou um trabalhador de sincronização de tabela</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>OID da relação que o trabalhador está sincronizando; NULL para o líder aplique trabalhador e aplique paralelamente os trabalhadores</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Última localização do log de pré-escrita recebida, sendo o valor inicial deste campo 0; NULL para trabalhadores de aplicação paralela</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Tempo da última mensagem recebida da origem do remetente WAL; NULL para aplicar trabalhadores em paralelo</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Tempo de recebimento da última mensagem recebida do remetente de origem WAL; NULL para trabalhadores de aplicação paralela</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Última localização do log de pré-escrita relatada ao remetente WAL de origem; NULL para trabalhadores de aplicação paralela</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Tempo da última localização do log de pré-escrita relatada ao remetente WAL de origem; NULL para trabalhadores de aplicação paralela</p>
</td>
</tr>
</tbody>
</table>
...
<p>
     ...
     and the corresponding conflict
       count (e.g.,
     <code class="literal">
      confl_*
     </code>
...
<p>ID de processo de um servidor de banco de dados ou processo de emissor WAL</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Verdadeiro se SSL é usado nesta conexão</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Versão do SSL em uso, ou NULL se o SSL não estiver em uso nesta conexão</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Nome do cifrador SSL em uso, ou NULL se o SSL não estiver em uso nesta conexão</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de bits no algoritmo de criptografia utilizado, ou NULL se SSL não for utilizado nesta conexão</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Campo Nome Distinguido (DN) do certificado do cliente utilizado, ou NULL se nenhum certificado do cliente foi fornecido ou se o SSL não está em uso nesta conexão. Este campo é truncado se o campo DN for mais longo que<code class="symbol">
      NAMEDATALEN
     </code>(64 caracteres em uma versão padrão).</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número serial do certificado do cliente, ou NULL se nenhum certificado do cliente foi fornecido ou se o SSL não está em uso nesta conexão. A combinação do número serial do certificado e do emissor do certificado identifica de forma única um certificado (a menos que o emissor reutilize erroneamente os números serial).</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>DN do emissor do certificado do cliente, ou NULL se não foi fornecido nenhum certificado do cliente ou se o SSL não está em uso nesta conexão. Este campo é truncado como<code class="structfield">
      client_dn
     </code>
     .
    </p>
...
<p>ID de processo de um backend</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Verdadeiro se a autenticação GSSAPI foi usada para esta conexão</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>O principal costuma autenticar essa conexão, ou NULL se o GSSAPI não foi usado para autenticar essa conexão. Esse campo é truncado se o principal for mais longo que<code class="symbol">
      NAMEDATALEN
     </code>(64 caracteres em uma versão padrão).</p>
</td>
</tr>
...
<p>Verdadeiro se a criptografia GSSAPI está em uso nesta conexão</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Verdadeiro se as credenciais do GSSAPI foram delegadas nesta conexão.</p>
</td>
</tr>
</tbody>
</table>
...
<p>Tipo de backend (por exemplo, trabalhador de fundo, trabalhador de autovacuum). Veja<a class="link" href="monitoring-stats.md#MONITORING-PG-STAT-ACTIVITY-VIEW" title="27.2.3. pg_stat_activity">
<code class="structname">
       pg_stat_activity
      </code>
</a>para mais informações sobre<code class="varname">
      backend_type
     </code>algumas s.<code class="varname">
      backend_type
     </code>Eles não acumulam estatísticas de operação de E/S e não serão incluídos na visualização.</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Objeto alvo de uma operação de E/S. Os valores possíveis são:</p>
<div class="itemizedlist">
<ul class="itemizedlist" style="list-style-type: disc; ">
<li class="listitem">
<p>
<code class="literal">
         relation
        </code>Relações permanentes.</p>
</li>
<li class="listitem">
<p>
<code class="literal">
         temp relation
        </code>Relações temporárias.</p>
</li>
<li class="listitem">
<p>
<code class="literal">
         wal
        </code>: Registros de escrita antecipada.</p>
</li>
</ul>
</div>
<p>
...
<p>O contexto de uma operação de E/S. Os valores possíveis são:</p>
<div class="itemizedlist">
<ul class="itemizedlist" style="list-style-type: disc; ">
<li class="listitem">
<p>
<code class="literal">
         normal
        </code>: O padrão ou padrão<code class="varname">
         context
        </code>para um tipo de operação de E/S. Por exemplo, por padrão, os dados da relação são lidos e escritos em buffers compartilhados. Assim, as leituras e escritas de dados da relação em e a partir de buffers compartilhados são rastreadas em<code class="varname">
         context
        </code>
<code class="literal">
         normal
...
<code class="literal">
         init
        </code>: As operações de E/S realizadas durante a criação dos segmentos WAL são rastreadas em<code class="varname">
         context
        </code>
<code class="literal">
         init
...
<code class="literal">
         vacuum
        </code>: Operações de E/S realizadas fora dos buffers compartilhados durante a limpeza e análise de relações permanentes. Os vazamentos temporários de tabela utilizam o mesmo conjunto de buffers locais que outras operações de E/S de tabela temporária e são rastreados em<code class="varname">
         context
        </code>
<code class="literal">
         normal
...
<code class="literal">
         bulkread
        </code>: Algumas operações de leitura de grande porte realizadas fora dos buffers compartilhados, por exemplo, uma varredura sequencial de uma grande tabela.</p>
</li>
<li class="listitem">
<p>
<code class="literal">
         bulkwrite
        </code>: Algumas operações de escrita de grande porte realizadas fora dos buffers compartilhados, como<code class="command">
         COPY
        </code>
        .
       </p>
...
<p>Número de operações de leitura.</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>O tamanho total das operações de leitura em bytes.</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Tempo gasto esperando operações de leitura em milissegundos (se<a class="xref" href="runtime-config-statistics.md#GUC-TRACK-IO-TIMING">
      track_io_timing
     </a>está habilitado e<code class="varname">
      object
     </code>não é<code class="literal">
      wal
     </code>, ou se<a class="xref" href="runtime-config-statistics.md#GUC-TRACK-WAL-IO-TIMING">
      track_wal_io_timing
     </a>está habilitado e<code class="varname">
      object
     </code>é<code class="literal">
      wal
     </code>, caso contrário, zero)</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de operações de escrita.</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>O tamanho total das operações de escrita em bytes.</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Tempo gasto esperando operações de escrita em milissegundos (se<a class="xref" href="runtime-config-statistics.md#GUC-TRACK-IO-TIMING">
      track_io_timing
     </a>está habilitado e<code class="varname">
      object
     </code>não é<code class="literal">
...
<p>Número de unidades do tamanho<code class="symbol">
      BLCKSZ
     </code>(geralmente 8 kB) que o processo solicitou que o kernel escrevesse para armazenamento permanente.</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Tempo gasto esperando operações de writeback em milissegundos (se<a class="xref" href="runtime-config-statistics.md#GUC-TRACK-IO-TIMING">
      track_io_timing
     </a>se habilitada, caso contrário, zero). Isso inclui o tempo gasto em filas de solicitações de saída e, potencialmente, o tempo gasto para escrever os dados sujos.</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de operações de extensão de relação.</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>O tamanho total das operações de extensão de relação em bytes.</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Tempo gasto esperando operações de extensão em milissegundos. (se<a class="xref" href="runtime-config-statistics.md#GUC-TRACK-IO-TIMING">
      track_io_timing
     </a>está habilitado e<code class="varname">
      object
     </code>não é<code class="literal">
...
<p>O número de vezes em que um bloco desejado foi encontrado em um buffer compartilhado.</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de vezes que um bloco foi escrito de um buffer compartilhado ou local para torná-lo disponível para outro uso.</p>
<p>Em<code class="varname">
      context
     </code>
<code class="literal">
      normal
     </code>, isso conta o número de vezes que um bloco foi ejetado de um buffer e substituído por outro bloco. Em<code class="varname">
      context
     </code>s<code class="literal">
      bulkwrite
     </code>,<code class="literal">
      bulkread
     </code>, e<code class="literal">
      vacuum
     </code>, isso conta o número de vezes que um bloco foi excluído dos buffers compartilhados, a fim de adicionar o buffer compartilhado a um buffer de anel separado, com tamanho limitado, para uso em uma operação de E/S em massa.</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>O número de vezes que um buffer existente em um buffer de anel limitado por tamanho, fora dos buffers compartilhados, foi reutilizado como parte de uma operação de E/S no<code class="literal">
      bulkread
     </code>,<code class="literal">
      bulkwrite
...
<p>Número de<code class="literal">
      fsync
     </code>chamadas. Essas são apenas rastreadas em<code class="varname">
      context
     </code>
<code class="literal">
      normal
...
<p>Tempo gasto esperando operações de fsync em milissegundos (se<a class="xref" href="runtime-config-statistics.md#GUC-TRACK-IO-TIMING">
      track_io_timing
     </a>está habilitado e<code class="varname">
      object
     </code>não é<code class="literal">
...
<p>O horário em que essas estatísticas foram redefinidas pela última vez.</p>
</td>
</tr>
</tbody>
</table>
...
<p>
     ...
     for shared objects.  This is the only column
       in this view that returns a value reflecting current state; all other columns return the accumulated values since the last reset.
    </p>
</td>
</tr>
<tr>
...
<p>Número de consultas neste banco de dados que foram canceladas devido ao espaço de tabela descartado</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de consultas nesse banco de dados que foram canceladas devido a tempos de bloqueio</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de consultas neste banco de dados que foram canceladas devido a instantâneos antigos</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de consultas neste banco de dados que foram canceladas devido a buffers fixados</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de consultas nesse banco de dados que foram canceladas devido a deadlocks</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de usos de slots lógicos neste banco de dados que foram cancelados devido a instantâneos antigos ou a um número muito baixo de<a class="xref" href="runtime-config-wal.md#GUC-WAL-LEVEL">
      wal_level
     </a>sobre o primário</p>
</td>
</tr>
</tbody>
</table>
...
<em>
     ...
     heap page, leaving behind an original
       version with a
...
<p>OID da tabela para este índice</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Nome do esquema no qual este índice está</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Nome da tabela para este índice</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de varreduras de índice iniciadas neste índice</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>O horário da última varredura deste índice, com base no horário mais recente de parada da transação</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de entradas de índice retornadas por varreduras neste índice</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de linhas de tabela em execução obtido por varreduras de índice simples usando este índice</p>
</td>
</tr>
</tbody>
</table>
...
Isso pode acontecer com consultas que utilizam certas construções SQL para procurar linhas que correspondem a qualquer valor de uma lista ou matriz de múltiplos valores escalares (consulte [Seção 9.25] [(functions-comparisons.md "9.25. Row and Array Comparisons")]). Também pode acontecer com consultas que possuem uma construção `column_name = value1 OR column_name = value2 ...`, embora apenas quando o otimizador transforme a construção em uma representação equivalente de matriz de múltiplos valores. Da mesma forma, quando as varreduras de índice B-tree utilizam a otimização de varredura de salto, uma pesquisa de índice é realizada cada vez que a varredura é reposicionada para a próxima página da folha de índice que pode ter tuplas correspondentes (consulte [Seção 11.3] [(indexes-multicolumn.md "11.3. Multicolumn Indexes")]).

### DICA

`EXPLAIN ANALYZE` exibe o número total de pesquisas de índice realizadas por cada nó de varredura de índice. Consulte [Seção 14.1.2][(using-explain.md#USING-EXPLAIN-ANALYZE "14.1.2. EXPLAIN ANALYZE")] para um exemplo que demonstra como isso funciona.
...
<p>Nome do esquema em que a tabela está</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de blocos de disco lidos a partir desta tabela</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de acertos de buffer nesta tabela</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de blocos de disco lidos de todos os índices nesta tabela</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de acertos de buffer em todos os índices nesta tabela</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de blocos de disco lidos da tabela TOAST desta tabela (se houver)</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de acertos de buffer na tabela TOAST desta tabela (se houver)</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de blocos de disco lidos dos índices da tabela TOAST da tabela (se houver)</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de acertos de buffer nos índices de tabela TOAST desta tabela (se houver)</p>
</td>
</tr>
</tbody>
</table>
...
<p>Número de blocos de disco lidos a partir deste índice</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de acertos de buffer neste índice</p>
</td>
</tr>
</tbody>
</table>
...
<p>Nome do esquema no qual essa sequência está</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de blocos de disco lidos a partir dessa sequência</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de acertos de buffer nesta sequência</p>
</td>
</tr>
</tbody>
</table>
...
<p>Nome do esquema no qual essa função está</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de vezes que essa função foi chamada</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Tempo total gasto nesta função e em todas as outras funções chamadas por ela, em milissegundos</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Tempo total gasto nesta função, sem incluir outras funções chamadas por ela, em milissegundos</p>
</td>
</tr>
</tbody>
</table>
...
<p>Número de blocos zerados durante as inicializações</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de vezes em que blocos de disco foram encontrados já no SLRU, de modo que uma leitura não fosse necessária (isto inclui apenas acertos no SLRU, não o cache do sistema de arquivos do sistema operacional)</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de blocos de disco lidos para este SLRU</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de blocos de disco escritos para este SLRU</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de blocos verificados quanto à existência para este SLRU</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de lavagens de dados sujos para este SLRU</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Número de truncos para este SLRU</p>
</td>
</tr>
<tr>
<td class="catalog_table_entry">
...
<p>Tempo em que essas estatísticas foram redefinidas pela última vez</p>
</td>
</tr>
</tbody>
</table>
...
Funções adicionais relacionadas ao sistema de estatísticas acumuladas estão listadas em [Tabela 27.36][(monitoring-stats.md#MONITORING-STATS-FUNCS-TABLE "Table 27.36. Additional Statistics Functions")].
...
<code class="literal">
        ...
        : Reset all the counters shown in
          the
        <code class="structname">
         pg_stat_recovery_prefetch
        </code>
...
<code class="literal">
        ...
        or not specified: All the counters from the
...
<code>
     ...
     or is not specified, all the counters shown in
        the
     <code class="structname">
      pg_stat_slru
     </code>
