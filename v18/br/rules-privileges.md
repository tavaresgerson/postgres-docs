...
Por exemplo, um índice não pode ser selecionado para consultas em visualizações de barreiras de segurança (ou tabelas com políticas de segurança a nível de linha) se um operador usado na cláusula `WHERE` estiver associado à família de operadores do índice, mas sua função subjacente não estiver marcada `LEAKPROOF`. O meta-comando `\dAo+` do programa [psql][(app-psql.md "psql")] é útil para listar famílias de operadores e determinar quais de seus operadores estão marcados como à prova de vazamento.
