...
O tipo de dados `xml` pode ser usado para armazenar dados XML. Sua vantagem em relação ao armazenamento de dados XML em um campo `text` é que ele verifica os valores de entrada quanto à forma bem formada, e há funções de suporte para realizar operações tipo-seguras sobre ele; veja [Seção 9.15][(functions-xml.md "9.15. XML Functions")]. O uso deste tipo de dados requer que a instalação tenha sido construída com `configure --with-libxml`.
...
Limites e notas de compatibilidade para o tipo de dados `xml` podem ser encontrados em [Seção D.3][(xml-limits-conformance.md "D.3. XML Limits and Conformance to SQL/XML")].
