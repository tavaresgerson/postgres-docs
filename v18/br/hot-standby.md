...
Vários parâmetros foram mencionados acima em [Seção 26.4.2][(hot-standby.md#HOT-STANDBY-CONFLICT "26.4.2. Handling Query Conflicts")] e [Seção 26.4.3][(hot-standby.md#HOT-STANDBY-ADMIN "26.4.3. Administrator's Overview")].
...
* O nível de isolamento de transação serializável ainda não está disponível no standby quente. (Consulte [Seção 13.2.3][(transaction-iso.md#XACT-SERIALIZABLE "13.2.3. Serializable Isolation Level")] e [Seção 13.4.1][(applevel-consistency.md#SERIALIZABLE-CONSISTENCY "13.4.1. Enforcing Consistency with Serializable Transactions")] para detalhes.) Uma tentativa de definir uma transação para o nível de isolamento serializável no modo de standby quente gerará um erro.