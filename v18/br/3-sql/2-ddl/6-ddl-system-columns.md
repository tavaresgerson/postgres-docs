...
Os identificadores de transação também são quantidades de 32 bits. Em um banco de dados de longa duração, é possível que os IDs de transação se repitam. Isso não é um problema fatal, dado os procedimentos de manutenção adequados; consulte [Capítulo 24](maintenance.md) para detalhes. No entanto, é imprudente depender da unicidade dos IDs de transação a longo prazo (mais de um bilhão de transações).
