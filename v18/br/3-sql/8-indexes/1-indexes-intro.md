...
Após a criação de um índice, o sistema precisa mantê-lo sincronizado com a tabela. Isso adiciona sobrecarga às operações de manipulação de dados. Os índices também podem impedir a criação de tuplas [somente heap](storage-hot.md)"). Portanto, os índices que raramente ou nunca são usados em consultas devem ser removidos.