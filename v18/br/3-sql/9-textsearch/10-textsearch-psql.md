## 12.10. Suporte ao psql [#](#TEXTSEARCH-PSQL)

Informações sobre os objetos de configuração de pesquisa de texto podem ser obtidas em psql usando um conjunto de comandos:

```
\dF{d,p,t}[+] [PATTERN]
```

Uma opção `+` produz mais detalhes.

O parâmetro opcional *`PATTERN`* pode ser o nome de um objeto de pesquisa de texto, opcionalmente qualificado por esquema. Se *`PATTERN`* for omitido, as informações sobre todos os objetos visíveis serão exibidas. *`PATTERN`* pode ser uma expressão regular e pode fornecer *padrões separados* para os nomes do esquema e do objeto. Os exemplos a seguir ilustram isso:
...
`\dF[+] [PATTERN]`: Configurações de busca de texto (adicionar `+` para mais detalhes).

```
=> \dF russian List of text search configurations Schema   |  Name   |            Description ------------+---------+------------------------------------ pg_catalog | russian | configuration for russian language
...
`\dFd[+] [PATTERN]`: Dicionários de busca de texto de CID-14 (adicionar `+` para mais detalhes).
...
`\dFp[+] [PATTERN]`: Parâmetros de busca de texto `+` (adicione para mais detalhes).

```
=> \dFp List of text search parsers Schema   |  Name   |     Description ------------+---------+--------------------- pg_catalog | default | default word parser => \dFp+ Text search parser "pg_catalog.default" Method      |    Function    | Description -----------------+----------------+------------- Start parse     | prsd_start     | Get next token  | prsd_nexttoken | End parse       | prsd_end       | Get headline    | prsd_headline  | Get token types | prsd_lextype   |
...
`\dFt[+] [PATTERN]`: Modelos de busca de texto (adicione `+` para mais detalhes).
