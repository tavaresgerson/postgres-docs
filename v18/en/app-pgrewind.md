...
pg_rewind requires that the target server either has the [wal_log_hints](runtime-config-wal.md#GUC-WAL-LOG-HINTS) option enabled in `postgresql.conf` or data checksums enabled when the cluster was initialized with initdb (the default). [full_page_writes](runtime-config-wal.md#GUC-FULL-PAGE-WRITES) must also be set to `on`, but is enabled by default.
...
`-D directory` `--target-pgdata=directory`: This option specifies the target data directory that is synchronized with the source. The target server must be shut down cleanly before running pg_rewind

`--source-pgdata=directory`: Specifies the file system path to the data directory of the source server to synchronize the target with. This option requires the source server to be cleanly shut down.
...
`-R` `--write-recovery-conf`: Create `standby.signal` and append connection settings to `postgresql.auto.conf` in the output directory. The dbname will be recorded only if the dbname was specified explicitly in the connection string or [environment variable](libpq-envars.md "32.15. Environment Variables"). `--source-server` is mandatory with this option.

`-n` `--dry-run`: Do everything except actually modifying the target directory.

`-N` `--no-sync`: By default, `pg_rewind` will wait for all files to be written safely to disk. This option causes `pg_rewind` to return without waiting, which is faster, but means that a subsequent operating system crash can leave the data directory corrupt. Generally, this option is useful for testing but should not be used on a production installation.

`-P` `--progress`: Enables progress reporting. Turning this on will deliver an approximate progress report while copying data from the source cluster.
