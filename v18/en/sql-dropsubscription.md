...
`IF EXISTS`: Do not throw an error if the subscription does not exist. A notice is issued in this case.

*`name`*: The name of a subscription to be dropped.

`CASCADE` `RESTRICT`: These key words do not have any effect, since there are no dependencies on subscriptions.
