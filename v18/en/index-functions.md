...
Estimate the costs of an index scan. This function is described fully in [Section 63.6](index-cost-estimation.md "63.6. Index Cost Estimation Functions"), below.
...
The purpose of an index, of course, is to support scans for tuples matching an indexable `WHERE` condition, often called a *qualifier* or *scan key*. The semantics of index scanning are described more fully in [Section 63.3](index-scanning.md "63.3. Index Scanning"), below. An index access method can support “plain” index scans, “bitmap” index scans, or both. The scan-related functions that an index access method must or may provide are:
