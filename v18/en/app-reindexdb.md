...
`--concurrently`: Use the `CONCURRENTLY` option. See [REINDEX](sql-reindex.md "REINDEX"), where all the caveats of this option are explained in detail.
...
reindexdb will open *`njobs`* connections to the database, so make sure your [max_connections](runtime-config-connection.md#GUC-MAX-CONNECTIONS) setting is high enough to accommodate all connections.
...
`-W` `--password`: Force reindexdb to prompt for a password before connecting to a database.
...
`--maintenance-db=dbname`: When the `-a`/`--all` is used, connect to this database to gather the list of databases to reindex. If not specified, the `postgres` database will be used, or if that does not exist, `template1` will be used. This can be a [connection string](libpq-connect.md#LIBPQ-CONNSTRING "32.1.1. Connection Strings"). If so, connection string parameters will override any conflicting command line options. Also, connection string parameters other than the database name itself will be re-used when connecting to other databases.
