...
`PQconnectdbParams` [#](#LIBPQ-PQCONNECTDBPARAMS): Makes a new connection to the database server.
...
`PQconnectdb` [#](#LIBPQ-PQCONNECTDB): Makes a new connection to the database server.
...
``` PGconn *PQconnectdb(const char *conninfo);
    ...
    See [Section 32.1.1](libpq-connect.md#LIBPQ-CONNSTRING "32.1.1. Connection Strings") for details.
...
:   Make a connection to the database server in a nonblocking manner.
    ...
    * The `hostaddr` parameter must be used appropriately
      ...
      this parameter in [Section 32.1.2](libpq-connect.md#LIBPQ-PARAMKEYWORDS "32.1.2. Parameter Key Words") for details.
...
:   `PQsetSSLKeyPassHook_OpenSSL` lets an application override
    libpq's [default
    handling of encrypted client certificate key files](libpq-ssl.md#LIBPQ-SSL-CLIENTCERT "32.19.2. Client Certificates") using
...
[Section 32.1.2](libpq-connect.md#LIBPQ-PARAMKEYWORDS "32.1.2. Parameter Key Words"), except that for compatibility
...
:   Name of host to connect to. If a host name looks like an absolute path
    ...
    [Section 32.1.1.3](libpq-connect.md#LIBPQ-MULTIPLE-HOSTS "32.1.1.3. Specifying Multiple Hosts") for details.
...
:   Numeric IP address of host to connect to. This should be in the
    ...
    * If both `host` and `hostaddr` are specified,
    ...
    [Section 32.16](libpq-pgpass.md "32.16. The Password File")).
...
:   The database name. Defaults to be the same as the user name.
    ...
    formats; see [Section 32.1.1](libpq-connect.md#LIBPQ-CONNSTRING "32.1.1. Connection Strings") for more details on
    those.
...
:   Specifies the name of the file used to store passwords
    (see [Section 32.16](libpq-pgpass.md "32.16. The Password File")).
...
:   Specifies the authentication method that the client requires from the
    ...
    :   The server must request MD5 hashed password authentication.
        ...
        PostgreSQL. Refer to
        [Section 20.5](auth-password.md "20.5. Password Authentication") for details about migrating to
...
:   Specifies command-line options to send to the server at connection
    ...
    options, consult [Chapter 19](runtime-config.md "Chapter 19. Server Configuration").

`application_name` [#](#LIBPQ-CONNECT-APPLICATION-NAME)
:   Specifies a value for the [application_name](runtime-config-logging.md#GUC-APPLICATION-NAME)
    configuration parameter.

`fallback_application_name` [#](#LIBPQ-CONNECT-FALLBACK-APPLICATION-NAME)
:   Specifies a fallback value for the [application_name](runtime-config-logging.md#GUC-APPLICATION-NAME) configuration parameter.
...
:   This option determines whether the connection should use the
    ...
    replication protocol, consult [Section 54.4](protocol-replication.md "54.4. Streaming Replication Protocol").
...
:   This option determines whether or with what priority a secure
    ...
    See [Section 32.19](libpq-ssl.md "32.19. SSL Support") for a detailed description of how
    these options work.
...
:   This parameter specifies the operating-system user name of the
    ...
    [Section 20.9](auth-peer.md "20.9. Peer Authentication").
...
:   Kerberos service name to use when authenticating with GSSAPI.
    ...
    [Section 20.6](gssapi-auth.md "20.6. GSSAPI Authentication").)
...
:   The base64-encoded SCRAM client key. This can be used by foreign-data
    ...
    authentication. See [Section F.38.1.10](postgres-fdw.md#POSTGRES-FDW-OPTIONS-CONNECTION-MANAGEMENT "F.38.1.10. Connection Management Options") for one such
...
:   Service name to use for additional parameters. It specifies a service
    ...
    can be centrally maintained. See [Section 32.17](libpq-pgservice.md "32.17. The Connection Service File").
...
:   The HTTPS URL of a trusted issuer to contact if the server requests an
    ...
    setting in [the server's HBA configuration](auth-oauth.md "20.15. OAuth Authorization/Authentication").
    ...
    but a [custom OAuth flow](libpq-oauth.md#LIBPQ-OAUTH-AUTHDATA-HOOKS "32.20.1. Authdata Hooks")
...
:   An OAuth 2.0 client identifier, as issued by the authorization server.
    ...
    [requests an OAuth token](auth-oauth.md "20.15. OAuth Authorization/Authentication") for the
    connection (and if no [custom
    OAuth hook](libpq-oauth.md#LIBPQ-OAUTH-AUTHDATA-HOOKS "32.20.1. Authdata Hooks") is installed to provide one), then this parameter must
