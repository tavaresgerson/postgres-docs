...
`IF EXISTS`: Do not throw an error if the server does not exist. A notice is issued in this case.

*`name`*: The name of an existing server.

`CASCADE`: Automatically drop objects that depend on the server (such as user mappings), and in turn all objects that depend on those objects (see [Section 5.15](ddl-depend.md "5.15. Dependency Tracking")).

`RESTRICT`: Refuse to drop the server if any objects depend on it. This is the default.
