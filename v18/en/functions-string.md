  ...
  <tr>
   <td class="func_table_entry">
    ...
    <p>
     ...
     .  This expression can
        only be used when the server encoding is
     <code class="literal">
      UTF8
     </code>
  ...
  <tr>
   <td class="func_table_entry">
    ...
    <p>
     ...
     can be used for Unicode Default Caseless
    ...
    <p>
     ...
     provider doesn't support case folding, so
     <code class="function">
      casefold
     </code>
     is identical to
  ...
  <tr>
   <td class="func_table_entry">
    ...
    <p>
     ...
     is zero, with the search beginning at the
     <em class="parameter">
      <code>
       start
      </code>
     ...
     is omitted, it defaults to 1.  See
     <a class="xref" href="functions-matching.md#FUNCTIONS-POSIX-REGEXP" title="9.7.3. POSIX Regular Expressions">
      Section 9.7.3
     </a>
     .
...
*`position`* (optional): A string of the form `n$` where *`n`* is the index of the argument to print. Index 1 means the first argument after *`formatstr`*. If the *`position`* is omitted, the default is to use the next argument in sequence.

*`flags`* (optional): Additional options controlling how the format specifier's output is formatted. Currently the only supported flag is a minus sign (`-`) which will cause the format specifier's output to be left-justified. This has no effect unless the *`width`* field is also specified.
