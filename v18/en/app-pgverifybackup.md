...
`-F format` `--format=format`: Specifies the format of the backup. *`format`* can be one of the following:

`p` `plain` :   Backup consists of plain files with the same layout as the source server's data directory and tablespaces.
...
`-P` `--progress`: Enable progress reporting. Turning this on will deliver a progress report while verifying checksums.
