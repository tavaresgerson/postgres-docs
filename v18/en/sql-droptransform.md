...
`IF EXISTS`: Do not throw an error if the transform does not exist. A notice is issued in this case.

*`type_name`*: The name of the data type of the transform.

*`lang_name`*: The name of the language of the transform.

`CASCADE`: Automatically drop objects that depend on the transform, and in turn all objects that depend on those objects (see [Section 5.15](ddl-depend.md "5.15. Dependency Tracking")).

`RESTRICT`: Refuse to drop the transform if any objects depend on it. This is the default.
