...
`PQputCopyData` [#](#LIBPQ-PQPUTCOPYDATA): Sends data to the server during `COPY_IN` state.
...
`PQputCopyEnd` [#](#LIBPQ-PQPUTCOPYEND): Sends end-of-data indication to the server during `COPY_IN` state.
