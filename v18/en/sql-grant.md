...
`SELECT` `INSERT` `UPDATE` `DELETE` `TRUNCATE` `REFERENCES` `TRIGGER` `CREATE` `CONNECT` `TEMPORARY` `EXECUTE` `USAGE` `SET` `ALTER SYSTEM` `MAINTAIN`: Specific types of privileges, as defined in [Section 5.8](ddl-priv.md "5.8. Privileges").

`TEMP`: Alternative spelling for `TEMPORARY`.
