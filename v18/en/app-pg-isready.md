...
`-d dbname` `--dbname=dbname`: Specifies the name of the database to connect to. The *`dbname`* can be a [connection string](libpq-connect.md#LIBPQ-CONNSTRING "32.1.1. Connection Strings"). If so, connection string parameters will override any conflicting command line options.
...
`-q` `--quiet`: Do not display status message. This is useful when scripting.

`-t seconds` `--timeout=seconds`: The maximum number of seconds to wait when attempting connection before returning that the server is not responding. Setting to 0 disables. The default is 3 seconds.

`-U username` `--username=username`: Connect to the database as the user *`username`* instead of the default.

`-V` `--version`: Print the pg_isready version and exit.
