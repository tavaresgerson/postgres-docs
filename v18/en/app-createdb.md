...
`-E encoding` `--encoding=encoding`: Specifies the character encoding scheme to be used in this database. The character sets supported by the PostgreSQL server are described in [Section 23.3.1](multibyte.md#MULTIBYTE-CHARSET-SUPPORTED "23.3.1. Supported Character Sets").

`-l locale` `--locale=locale`: Specifies the locale to be used in this database. This is equivalent to specifying `--lc-collate`, `--lc-ctype`, and `--icu-locale` to the same value. Some locales are only valid for ICU and must be set with `--icu-locale`.
...
`--builtin-locale=locale`: Specifies the locale name when the builtin provider is used. Locale support is described in [Section 23.1](locale.md "23.1. Locale Support").

`--icu-locale=locale`: Specifies the ICU locale ID to be used in this database, if the ICU locale provider is selected.

`--icu-rules=rules`: Specifies additional collation rules to customize the behavior of the default collation of this database. This is supported for ICU only.

`--locale-provider={builtin|libc|icu}`: Specifies the locale provider for the database's default collation.
...
`-S strategy` `--strategy=strategy`: Specifies the database creation strategy. See [CREATE DATABASE STRATEGY](sql-createdatabase.md#CREATE-DATABASE-STRATEGY) for more details.
...
`--maintenance-db=dbname`: Specifies the name of the database to connect to when creating the new database. If not specified, the `postgres` database will be used; if that does not exist (or if it is the name of the new database being created), `template1` will be used. This can be a [connection string](libpq-connect.md#LIBPQ-CONNSTRING "32.1.1. Connection Strings"). If so, connection string parameters will override any conflicting command line options.
