...
*`name`*: The name of a run-time parameter. Available parameters are documented in [Chapter 19](runtime-config.md "Chapter 19. Server Configuration") and on the [SET](sql-set.md "SET") reference page. In addition, there are a few parameters that can be shown but not set:
...
The function `current_setting` produces equivalent output; see [Section 9.28.1](functions-admin.md#FUNCTIONS-ADMIN-SET "9.28.1. Configuration Settings Functions"). Also, the [`pg_settings`](view-pg-settings.md "53.25. pg_settings") system view produces the same information.
