...
`BGWORKER_SHMEM_ACCESS`: Requests shared memory access. This flag is required.

`BGWORKER_BACKEND_DATABASE_CONNECTION`: Requests the ability to establish a database connection through which it can later run transactions and queries. A background worker using `BGWORKER_BACKEND_DATABASE_CONNECTION` to connect to a database must also attach shared memory using `BGWORKER_SHMEM_ACCESS`, or worker start-up will fail.
