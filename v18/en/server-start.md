...
does *not* mean you've run out of disk space. It means your kernel's limit on the number of System V semaphores is smaller than the number PostgreSQL wants to create. As above, you might be able to work around the problem by starting the server with a reduced number of allowed connections ([max_connections](runtime-config-connection.md#GUC-MAX-CONNECTIONS)), but you'll eventually want to increase the kernel limit.
...
This is the generic “I couldn't find a server to talk to” failure. It looks like the above when TCP/IP communication is attempted. A common mistake is to forget to configure [listen_addresses](runtime-config-connection.md#GUC-LISTEN-ADDRESSES) so that the server accepts remote TCP connections.
