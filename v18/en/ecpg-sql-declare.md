...
*`cursor_name`* [#](#ECPG-SQL-DECLARE-CURSOR-NAME): A cursor name, case sensitive. This can be an SQL identifier or a host variable.

*`prepared_name`* [#](#ECPG-SQL-DECLARE-PREPARED-NAME): The name of a prepared query, either as an SQL identifier or a host variable.

*`query`* [#](#ECPG-SQL-DECLARE-QUERY): A [SELECT](sql-select.md "SELECT") or [VALUES](sql-values.md "VALUES") command which will provide the rows to be returned by the cursor.
