...
`PQexec` [#](#LIBPQ-PQEXEC): Submits a command to the server and waits for the result.
...
:   Submits a request to create a prepared statement with the
    ...
    planned each time; see [PREPARE](sql-prepare.md "PREPARE") for details.
...
be created by executing SQL [PREPARE](sql-prepare.md "PREPARE")
statements.
...
:   Returns the result status of the command.
    ...
    :   The `PGresult` contains a single result tuple
        ...
        (see [Section 32.6](libpq-single-row-mode.md "32.6. Retrieving Query Results in Chunks")).
    ...
    to the notice processor (see [Section 32.13](libpq-notice-processing.md "32.13. Notice Processing")).
...
:   Returns an individual field of an error report.
    ...
    *`fieldcode`* is an error field identifier; see the symbols
    ...
    :   The SQLSTATE code for the error. The SQLSTATE code identifies
        ...
        For a list of the possible SQLSTATE codes, see [Appendix A](errcodes-appendix.md "Appendix A. PostgreSQL Error Codes"). This field is not localizable,
    ...
    types; see [Appendix A](errcodes-appendix.md "Appendix A. PostgreSQL Error Codes"). Do not assume that
...
:   ```
    ...
    [`PQescapeLiteral`](libpq-exec.md#LIBPQ-PQESCAPELITERAL) returns an escaped version of the *`str`* parameter in memory allocated with `malloc()`. This memory should be freed using `PQfreemem()` when the result is no longer needed. A terminating zero byte is not required, and should not be counted in *`length`*. (If a terminating zero byte is found before *`length`* bytes are processed, [`PQescapeLiteral`](libpq-exec.md#LIBPQ-PQESCAPELITERAL) stops at the zero; the behavior is thus rather like `strncpy`.) The return string has all special characters replaced so that they can be properly processed by the PostgreSQL string literal parser. A terminating zero byte is also added. The single quotes that must surround PostgreSQL string literals are included in the result string.

    On error, [`PQescapeLiteral`](libpq-exec.md#LIBPQ-PQESCAPELITERAL) returns `NULL` and a suitable message is stored in the *`conn`* object.
...
:   ```
    ...
    [`PQescapeStringConn`](libpq-exec.md#LIBPQ-PQESCAPESTRINGCONN) returns the number of bytes written to *`to`*, not including the terminating zero byte.

`PQescapeString` [#](#LIBPQ-PQESCAPESTRING): [`PQescapeString`](libpq-exec.md#LIBPQ-PQESCAPESTRING) is an older, deprecated version of [`PQescapeStringConn`](libpq-exec.md#LIBPQ-PQESCAPESTRINGCONN).
...
`PQescapeByteaConn` [#](#LIBPQ-PQESCAPEBYTEACONN): Escapes binary data for use within an SQL command with the type `bytea`. As with [`PQescapeStringConn`](libpq-exec.md#LIBPQ-PQESCAPESTRINGCONN), this is only used when inserting data directly into an SQL command string.
...
``` unsigned char *PQescapeByteaConn(PGconn *conn, const unsigned char *from, size_t from_length, size_t *to_length);
    ...
    either hex encoding or backslash escaping. See [Section 8.4](datatype-binary.md "8.4. Binary Data Types") for more information.
