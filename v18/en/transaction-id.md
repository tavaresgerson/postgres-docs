...
The internal transaction ID type `xid` is 32 bits wide and [wraps around](routine-vacuuming.md#VACUUM-FOR-WRAPAROUND "24.1.5. Preventing Transaction ID Wraparound Failures") every 4 billion transactions. A 32-bit epoch is incremented during each wraparound. There is also a 64-bit type `xid8` which includes this epoch and therefore does not wrap around during the life of an installation; it can be converted to xid by casting. The functions in [Table 9.84](functions-info.md#FUNCTIONS-PG-SNAPSHOT "Table 9.84. Transaction ID and Snapshot Information Functions") return `xid8` values. Xids are used as the basis for PostgreSQL's [MVCC](mvcc.md "Chapter 13. Concurrency Control") concurrency mechanism and streaming replication.
