...
`plpy.execute(query [, limit])`: Calling `plpy.execute` with a query string and an optional row limit argument causes that query to be run and the result to be returned in a result object.
...
``` rv = plpy.execute("SELECT * FROM my_table", 5)
    ...
    `nrows()` :   Returns the number of rows processed by the command. Note that this is not necessarily the same as the number of rows returned. For example, an `UPDATE` command will set this value but won't return any rows (unless `RETURNING` is used).
    ...
    `__str__()` :   The standard `__str__` method is defined so that it is possible for example to debug query execution results using `plpy.debug(rv)`.
...
`plpy.prepare(query [, argtypes])` `plpy.execute(plan [, arguments [, limit]])`: `plpy.prepare` prepares the execution plan for a query. It is called with a query string and a list of parameter types, if you have parameter references in the query. For example:
...
``` plan = plpy.prepare("SELECT last_name FROM my_users WHERE first_name = $1", ["text"])
    ...
    and Python data types as described in [Section 44.2](plpython-data.md "44.2. Data Values").

    When you prepare a plan using the PL/Python module it is automatically
    saved. Read the SPI documentation ([Chapter 45](spi.md "Chapter 45. Server Programming Interface")) for a
    ...
    [Section 44.3](plpython-sharing.md "44.3. Sharing Data")). For example:
...
to [Table A.1](errcodes-appendix.md#ERRCODES-TABLE "Table A.1. PostgreSQL Error Codes") for a list of possible
