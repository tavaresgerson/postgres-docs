...
`-l limit` `--limit=limit`: Remove no more than *`limit`* large objects per transaction (default 1000). Since the server acquires a lock per LO removed, removing too many LOs in one transaction risks exceeding [max_locks_per_transaction](runtime-config-locks.md#GUC-MAX-LOCKS-PER-TRANSACTION). Set the limit to zero if you want all removals done in a single transaction.

`-n` `--dry-run`: Don't remove anything, just show what would be done.

`-v` `--verbose`: Write a lot of progress messages.

`-V` `--version`: Print the vacuumlo version and exit.
...
`-W` `--password`: Force vacuumlo to prompt for a password before connecting to a database.
