...
`const char * name`: name for portal, or `NULL` to let the system select a name

`const char * command`: command string

`int nargs`: number of input parameters (`$1`, `$2`, etc.)

`Oid * argtypes`: an array of length *`nargs`*, containing the OIDs of the data types of the parameters

`Datum * values`: an array of length *`nargs`*, containing the actual parameter values

`const char * nulls`: an array of length *`nargs`*, describing which parameters are null
...
`bool read_only`: `true` for read-only execution

`int cursorOptions`: integer bit mask of cursor options; zero produces default behavior
