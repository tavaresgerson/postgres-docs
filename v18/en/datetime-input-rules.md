...
2. If the token is an alphabetic string, match up with possible strings:
   ...
   1. See if the token matches any known time zone abbreviation. These abbreviations are determined by the configuration settings described in [Section B.4](datetime-config-files.md "B.4. Date/Time Configuration Files").
