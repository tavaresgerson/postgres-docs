...
`-d duration` `--duration=duration`: Specifies the test duration, in seconds. Longer durations give slightly better accuracy, and are more likely to discover problems with the system clock moving backwards. The default test duration is 3 seconds.

`-V` `--version`: Print the pg_test_timing version and exit.
