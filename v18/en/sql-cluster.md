...
When a table is clustered, PostgreSQL remembers which index it was clustered by. The form `CLUSTER table_name` reclusters the table using the same index as before. You can also use the `CLUSTER` or `SET WITHOUT CLUSTER` forms of [`ALTER TABLE`](sql-altertable.md "ALTER TABLE") to set the index to be used for future cluster operations, or to clear any previous setting.
...
*`table_name`*: The name (possibly schema-qualified) of a table.

*`index_name`*: The name of an index.

`VERBOSE`: Prints a progress report as each table is clustered at `INFO` level.
