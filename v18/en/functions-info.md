...
[Table 9.71](functions-info.md#FUNCTIONS-INFO-SESSION-TABLE "Table 9.71. Session Information Functions") shows several functions that extract session and system information.
...
[Table 9.73](functions-info.md#FUNCTIONS-ACLITEM-OP-TABLE "Table 9.73. aclitem Operators") shows the operators available for the `aclitem` type, which is the catalog representation of access privileges. See [Section 5.8](ddl-priv.md "5.8. Privileges") for information about how to read access privilege values.
...
[Table 9.74](functions-info.md#FUNCTIONS-ACLITEM-FN-TABLE "Table 9.74. aclitem Functions") shows some additional functions to manage the `aclitem` type.
  ...
  <tr>
   <td class="func_table_entry">
    ...
    <p>
     ...
     .  This represents the access privileges that will be assumed when an object's
     <acronym class="acronym">
      ACL
     </acronym>
     entry is null. (The default access privileges
        are described in
  ...
  <tr>
   <td class="func_table_entry">
    ...
    <p>
     ...
     , etc, all of which are set in the
        result.  (Case of the privilege string is not significant, and extra whitespace is allowed between but not within privilege names.)
    </p>
   </td>
  </tr>
 </tbody>
...
[Table 9.76](functions-info.md#FUNCTIONS-INFO-CATALOG-TABLE "Table 9.76. System Catalog Information Functions") lists functions that extract information from the system catalogs.
...
[Table 9.81](functions-info.md#FUNCTIONS-INFO-OBJECT-TABLE "Table 9.81. Object Information and Addressing Functions") lists functions related to database object identification and addressing.
...
The functions shown in [Table 9.82](functions-info.md#FUNCTIONS-INFO-COMMENT-TABLE "Table 9.82. Comment Information Functions") extract comments previously stored with the [COMMENT](sql-comment.md "COMMENT") command. A null value is returned if no comment could be found for the specified parameters.
...
The functions shown in [Table 9.83](functions-info.md#FUNCTIONS-INFO-VALIDITY-TABLE "Table 9.83. Data Validity Checking Functions") can be helpful for checking validity of proposed input data.
...
The functions shown in [Table 9.84](functions-info.md#FUNCTIONS-PG-SNAPSHOT "Table 9.84. Transaction ID and Snapshot Information Functions") provide server transaction information in an exportable form. The main use of these functions is to determine which transactions were committed between two snapshots.
...
The internal transaction ID type `xid` is 32 bits wide and wraps around every 4 billion transactions. However, the functions shown in [Table 9.84](functions-info.md#FUNCTIONS-PG-SNAPSHOT "Table 9.84. Transaction ID and Snapshot Information Functions"), except `age`, `mxid_age`, and `pg_get_multixact_members`, use a 64-bit type `xid8` that does not wrap around during the life of an installation and can be converted to `xid` by casting if required; see [Section 67.1](transaction-id.md "67.1. Transactions and Identifiers") for details. The data type `pg_snapshot` stores information about transaction ID visibility at a particular moment in time. Its components are described in [Table 9.85](functions-info.md#FUNCTIONS-PG-SNAPSHOT-PARTS "Table 9.85. Snapshot Components"). `pg_snapshot`'s textual representation is `xmin:xmax:xip_list`. For example `10:20:10,14,15` means `xmin=10, xmax=20, xip_list=10, 14, 15`.
...
The functions shown in [Table 9.93](functions-info.md#FUNCTIONS-VERSION "Table 9.93. Version Information Functions") print version information.
...
The functions shown in [Table 9.94](functions-info.md#FUNCTIONS-WAL-SUMMARY "Table 9.94. WAL Summarization Information Functions") print information about the status of WAL summarization. See [summarize_wal](runtime-config-wal.md#GUC-SUMMARIZE-WAL).
