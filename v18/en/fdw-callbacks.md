...
See [Section 58.4](fdw-planning.md "58.4. Foreign Data Wrapper Query Planning") for additional information.
...
If an FDW wishes to support *late row locking* (as described in [Section 58.5](fdw-row-locking.md "58.5. Row Locking in Foreign Data Wrappers")), it must provide the following callback functions:
...
See [Section 58.5](fdw-row-locking.md "58.5. Row Locking in Foreign Data Wrappers") for more information.
...
This function is called when [ANALYZE](sql-analyze.md "ANALYZE") is executed on a foreign table. If the FDW can collect statistics for this foreign table, it should return `true`, and provide a pointer to a function that will collect sample rows from the table in *`func`*, plus the estimated size of the table in pages in *`totalpages`*. Otherwise, return `false`.
