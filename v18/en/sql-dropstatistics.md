...
`IF EXISTS`: Do not throw an error if the statistics object does not exist. A notice is issued in this case.

*`name`*: The name (optionally schema-qualified) of the statistics object to drop.

`CASCADE` `RESTRICT`: These key words do not have any effect, since there are no dependencies on statistics.
