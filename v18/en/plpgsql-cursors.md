...
Before a cursor can be used to retrieve rows, it must be *opened*. (This is the equivalent action to the SQL command [`DECLARE CURSOR`](sql-declare.md "DECLARE").) PL/pgSQL has three forms of the `OPEN` statement, two of which use unbound cursor variables while the third uses a bound cursor variable.
