...
`pg_get_wal_record_info(in_lsn pg_lsn) returns record` [#](#PGWALINSPECT-FUNCS-PG-GET-WAL-RECORD-INFO): Gets WAL record information about a record that is located at or after the *`in_lsn`* argument. For example:
...
`pg_get_wal_records_info(start_lsn pg_lsn, end_lsn pg_lsn) returns setof record` [#](#PGWALINSPECT-FUNCS-PG-GET-WAL-RECORDS-INFO): Gets information of all the valid WAL records between *`start_lsn`* and *`end_lsn`*. Returns one row per WAL record. For example:
...
:   Gets information about each block reference from all the valid
    ...
    *`block_id`* values.
    ...
    [`pg_database`](catalog-pg-database.md "52.15. pg_database").`oid`, and
    ...
    [Table 9.103](functions-admin.md#FUNCTIONS-ADMIN-DBLOCATION "Table 9.103. Database Object Location Functions")) can help you to
