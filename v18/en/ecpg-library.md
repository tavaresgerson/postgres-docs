...
* `ECPGtransactionStatus(const char *connection_name)` returns the current transaction status of the given connection identified by *`connection_name`*. See [Section 32.2](libpq-status.md "32.2. Connection Status Functions") and libpq's [`PQtransactionStatus`](libpq-status.md#LIBPQ-PQTRANSACTIONSTATUS) for details about the returned status codes.
