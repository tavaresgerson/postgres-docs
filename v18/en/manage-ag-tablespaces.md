...
To determine the set of existing tablespaces, examine the [`pg_tablespace`](catalog-pg-tablespace.md "52.56. pg_tablespace") system catalog, for example

```
SELECT spcname, spcowner::regrole, pg_tablespace_location(oid) FROM pg_tablespace;
```

It is possible to find which databases use which tablespaces; see [Table 9.76](functions-info.md#FUNCTIONS-INFO-CATALOG-TABLE "Table 9.76. System Catalog Information Functions"). The [psql](app-psql.md "psql") program's `\db` meta-command is also useful for listing the existing tablespaces.
