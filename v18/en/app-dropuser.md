...
*`username`*: Specifies the name of the PostgreSQL user to be removed. You will be prompted for a name if none is specified on the command line and the `-i`/`--interactive` option is used.

`-e` `--echo`: Echo the commands that dropuser generates and sends to the server.

`-i` `--interactive`: Prompt for confirmation before actually removing the user, and prompt for the user name if none is specified on the command line.
...
`-W` `--password`: Force dropuser to prompt for a password before connecting to a database.
